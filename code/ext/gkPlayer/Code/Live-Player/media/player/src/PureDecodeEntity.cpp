...
TTInt CTTPureDecodeEntity::Decode(const TTChar* aUrl, const TTChar* OutPutPath)
{
	TTInt nRet = OpenAndParse(aUrl);
	if (TTKErrNone != nRet)
		return nRet;
...
TTInt CTTPureDecodeEntity::DecodeData(const TTChar* Outputpath)
{
...
_begin:
	...
	if(iCurSrcMediaFormat == TTAudioInfo::KTTMediaTypeAudioCodeWAV) {
		...
		if (dataSize == 0){
			fwrite(mSrcBuffer.pBuffer,1, mSrcBuffer.nSize,fp); 
		}
		else {
			fwrite(aDstBuffer->pBuffer,1, dataSize, fp); 
		}
...
__retry:
		...
		else if (nErr == TTKErrNone || nErr == TTKErrOverflow)
		{
			...
			if(mAudioDecSize >= mAudioStepSize) 
			{
				fwrite(pBuffer,1, mAudioDecSize, fp); 
				//iCopyPos += nFillSize;
				mAudioDecSize = 0;
				//decode one more
				goto __retry;
...
TTInt CTTPureDecodeEntity::GetDataFromSpecifiedTimePos(const TTChar* aUrl, TTInt timePos, TTInt duration)
{
	TTInt nRet = OpenAndParse(aUrl);
	if (TTKErrNone != nRet)
		return nRet;
	...
	//do seek
	TTInt pos = iMediaInfoProxy->Seek(timePos, 0);
	if ( pos < 0)
		return  TTKErrNotFound;

	if (TTKErrNone == nRet)
	{
		nRet = ExtractData(duration);
	}
...
TTInt CTTPureDecodeEntity::ExtractData(TTInt duration)
{
	TTInt nErr;
	TTBool init = ETTFalse; 
	TTInt  nStartTime = 0;
	iCopyPos = 0;
...
_begin:
	...
	if(!init){
		nStartTime = mSrcBuffer.llTime;
		init = ETTTrue;
	}

	if (nErr == TTKErrEof || nErr == TTKErrNone)
	{
		if (iCopyPos + mSrcBuffer.nSize >= MAX_DECPDE_DATA)
		{
			nErr = TTKErrNoMemory;
			goto _end;
		}

		duration -= (mSrcBuffer.llTime - nStartTime); 
		nStartTime = mSrcBuffer.llTime;
		//copy data
		memcpy(iDecodeDataPtr + iCopyPos, mSrcBuffer.pBuffer, mSrcBuffer.nSize);

		iCopyPos += mSrcBuffer.nSize;
