...
AMFProp_Encode(AMFObjectProperty *prop, char *pBuffer, char *pBufEnd)
{
    ...
    switch (prop->p_type)
    {
        ...
        default:
            LOGE("invalid type. %d", prop->p_type);
            pBuffer = NULL;
    };
...
AMF3ReadString(const char *data, AVal *str)
{
    ...
    {				/* reference: 0xxx */
        ...
        LOGI("string reference, index: %d, not supported, ignoring!", refIndex);
        return len;
    }
    else
    {
                ...
                int bDecodeName)
{
    ...
    if (nSize == 0 || !pBuffer)
    {
        LOGI("empty buffer/no buffer pointer!");
        return -1;
    }
    ...
    switch (type)
    {
        ...
        case AMF3_DATE:
        {
            ...
            {			/* reference */
                ...
                LOGI("AMF3_DATE reference: %d, not supported!", nIndex);
            }
            else
            {
                if (nSize < 8)
        ...
        default:
            LOGI("AMF3 unknown/unsupported datatype 0x%02x, @0x%08X", (unsigned char)(*pBuffer), pBuffer);
            return -1;
    }
               ...
               int bDecodeName)
{
    ...
    if (nSize == 0 || !pBuffer)
    {
        LOGI("AfmProc_dec: Empty buffer/no buffer pointer!");
        return -1;
    }
    
    if (bDecodeName && nSize < 4)
    {				/* at least name (length + at least 1 byte) and 1 byte of data */
        LOGI("AfmProc_dec: Not enough data for decoding with name, less than 4 bytes!");
        return -1;
    }
    ...
    if (bDecodeName)
    {
        ...
        {
            LOGI("AfmProc_dec: Name size out of range: namesize (%d) > len (%d) - 2", nNameSize, nSize);
            return -1;
        }
    ...
    switch (prop->p_type)
    {
        ...
        case AMF_REFERENCE:
        {
            LOGE("AMF_REFERENCE not supported!");
            return -1;
           // break;
        }
        case AMF_ECMA_ARRAY:
        ...
        case AMF_RECORDSET:
        {
            LOGE( "AMF_RECORDSET reserved!");
            return -1;
            //break;
        }
        case AMF_XML_DOC:
        {
            LOGE( "AMF_XML_DOC not supported!");
            return -1;
            //break;
        }
        case AMF_TYPED_OBJECT:
        {
            LOGE("AMF_TYPED_OBJECT not supported!");
            return -1;
            //break;
        }
        case AMF_AVMPLUS:
        ...
        default:
            LOGI("unknown datatype 0x%02x, @0x%08X",prop->p_type, pBuffer - 1);
            return -1;
    }
...
AMF_Encode(AMFObject *obj, char *pBuffer, char *pBufEnd)
{
    ...
    *pBuffer++ = AMF_OBJECT;
    ...
    for (i = 0; i < obj->o_num; i++)
    {
        ...
        if (res == NULL)
        {
            LOGE("AMF_Encode - failed to encode property in index %d",
                     i);
            break;
        }
        else
...
AMF3_Decode(AMFObject *obj, const char *pBuffer, int nSize, int bAMFData)
{
    ...
    if (bAMFData)
    {
        if (*pBuffer != AMF3_OBJECT)
            LOGE("AMF3 Object encapsulated in AMF stream does not start with AMF3_OBJECT!");
        pBuffer++;
        nSize--;
    }
    ...
    {				/* object reference, 0xxx */
        ...
        LOGI("Object reference, index: %d", objectIndex);
    }
    else				/* object instance */
    {
        ...
        {			/* class reference */
            ...
            LOGI("Class reference: %d", classIndex);
        }
        else
        {
            ...
            LOGI("Class name: %s, externalizable: %d, dynamic: %d, classMembers: %d",
            ...
            for (i = 0; i < cd.cd_num; i++)
            {
                ...
                LOGI("Member: %s", memberName.av_val);
        ...
        if (cd.cd_externalizable)
        {
            ...
            LOGI("Externalizable, TODO check");
            
            nRes = AMF3Prop_Decode(&prop, pBuffer, nSize, FALSE);
			if (nRes == -1){
                LOGI("1.failed to decode AMF3 property!")
			}
            else
            {
                nSize -= nRes;
        ...
        else
        {
            ...
            for (i = 0; i < cd.cd_num; i++)	/* non-dynamic */
            {
                nRes = AMF3Prop_Decode(&prop, pBuffer, nSize, FALSE);
                if (nRes == -1)
                    LOGI("2.failed to decode AMF3 property!");
...
AMF_Decode(AMFObject *obj, const char *pBuffer, int nSize, int bDecodeName)
{
    ...
    while (nSize > 0)
    {
        ...
        if (bError)
        {
            LOGE("DECODING ERROR, IGNORING BYTES UNTIL NEXT KNOWN PATTERN!");
            nSize--;
            pBuffer++;
            continue;
        }
