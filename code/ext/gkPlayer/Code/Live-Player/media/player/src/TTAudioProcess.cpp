/**
* File : TTAudioProcess.cpp  
* Created on : 2014-9-1
* Author : yongping.lin
* Description : TTAudioProcess实现文件
*/
...
TTInt TTCAudioProcess::updateParam()
{
	...
	if(mTDStretch == NULL) {
		mTDStretch = new TDStretch();
	}

	mTDStretch->setChannels(mAudioFormatOut.Channels);
	mTDStretch->setParameters(mAudioFormatOut.SampleRate, 40, 15, 8);
	mTDStretch->setTempo(1.05);
...
TTInt TTCAudioProcess::getOutputBuffer(TTBuffer* dstBuffer)
{
	...
	if(mAudioCount > 1) {
		...
		//LOGI("TTCAudioProcess::mListUsing %d, mListFull %d, SysDiff %d, FrameTime %lld, mCurBuffer->lReserve %d", mListUsing, mListFull, (TTInt)(GetTimeOfDay() - mLastSysTime), mCurBuffer->llTime, mCurBuffer->lReserve);
		if(mEffectEnable) {
#ifndef __TT_OS_IOS__
			doAudioEffect(mCurBuffer);
#endif
			updateWaveBuffer(mCurBuffer);	
		}
		...
		if(mEffectEnable) {
			doAudioEffect(dstBuffer);
			updateWaveBuffer(dstBuffer);
		}
	}
...
TTInt TTCAudioProcess::doTDStretchConv(TTBuffer* srcBuffer, TTBuffer* dstBuffer)
{
	TTInt32 outlen = 0;
	TTInt inlen = 0;
	TTInt outlenMax = 0;
	...
	if(inlen > 0) {
		outlen = mTDStretch->process((SAMPLETYPE *)input, inlen, (SAMPLETYPE *)output, outlenMax);
	}
...
TTInt TTCAudioProcess::doSampleConvert(TTBuffer* dstBuffer)
{
	...
	if(mDoTDStretch) {
		doTDStretchConv(mSrcBuffer, dstBuffer);
	}

	return TTKErrNone;
}
 
TTInt TTCAudioProcess::doAudioEffect(TTBuffer* dstBuffer)
{
#ifdef __MEDIA_PLAYER__
	CTTAudioEffectManager* pInstance = CTTAudioEffectManager::QueryInstance();
	if (NULL != pInstance && dstBuffer->lReserve == 0)	{
		pInstance->Process(dstBuffer->pBuffer, dstBuffer->nSize);
		dstBuffer->lReserve++;
	}
#endif
	return TTKErrNone;
...
TTInt TTCAudioProcess::resetAudioEffect()
{
#ifdef __MEDIA_PLAYER__
	GKCAutoLock Lock(&mCritical);
	CTTAudioEffectManager* pInstance = CTTAudioEffectManager::QueryInstance();
	if (NULL != pInstance)	{
		const effect_uuid_t * equUUID = &EQUALIZER_UUID_;
		const effect_uuid_t * bassUUID = &BASSBOOST_UUID_;
		const effect_uuid_t * treUUID = &TREBLEBOOST_UUID_;
		const effect_uuid_t * revUUID = &REVERB_AUX_UUID_;
		pInstance->Reset(equUUID);
		pInstance->Reset(bassUUID);
		pInstance->Reset(treUUID);
		pInstance->Reset(revUUID);
	}
#endif
	return TTKErrNone;
}
...
TTInt TTCAudioProcess::onAudioProc (TTInt nMsg, TTInt nVar1, TTInt nVar2, void* nVar3)
{
	...
	TTInt nDoTDStretchPre = mDoTDStretch;
	TTInt nDoTDStretchNow = mDoTDStretch;
	if(mSrcMux->GetParam(TT_PID_COMMON_DELAYSTATUS, &nDoTDStretchNow) == TTKErrNone) {
		if(nDoTDStretchPre == 1 && nDoTDStretchNow == 0) {
			SAMPLETYPE *Outsamples = (SAMPLETYPE *)(DstBuffer->pBuffer + DstBuffer->nSize);
			unsigned int  numOutSamples = (mAudioBufferSize - DstBuffer->nSize)/(sizeof(short)*mAudioFormatOut.Channels);
			TTInt nLeft = mTDStretch->getLeftBuffer(Outsamples, numOutSamples);
			if(nLeft > 0) {
				DstBuffer->nSize += nLeft*sizeof(short)*mAudioFormatOut.Channels;
			}
		} 
		mDoTDStretch = nDoTDStretchNow;
	}
