...
static const TTChar KContentType[] = {'C', 'o', 'n', 't', 'e', 'n', 't', '-', 'T', 'y', 'p', 'e', '\0'};
...
TTInt CTTHttpClient::Recv(TTChar* aDstBuffer, TTInt aSize)
{
   ...
   // TTInt retryCnt = HTTP_HEADER_RECV_MAXTIMES;// 6*5s = 30s, total wait time is 30s;
	...
	do{
        ...
        nOffset = GetTimeOfDay() - nStartTime;
        if (nOffset > HTTP_HEADER_RECV_MAXTIME || iCancel) {
            break;
        }
    }while (nErr == 0);
    return nErr;
...
TTInt CTTHttpClient::SendRequest(TTInt aPort, TTInt64 aOffset)
{
	memset(iRequset, 0, sizeof(iRequset));
	if (iHostMetaData)
	{
		if(NULL == strstr(iHostMetaData,"Host:"))
		{
			if (aOffset > 0)
			{
				if(aPort != 80) {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%sHost: %s:%d\r\nRange: bytes=%ld-\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, iHostAddr, aPort, aOffset);

				} else {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%sHost: %s\r\nRange: bytes=%ld-\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, iHostAddr, aOffset);
				}
			}
			else
			{
				if(aPort != 80) {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%sHost: %s:%d\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, iHostAddr, aPort);
				} else {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%sHost: %s\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, iHostAddr);
				}
			}
		}
		else{
			...
			{
				if(aPort != 80) {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%s:%d\r\nRange: bytes=%ld-\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, aPort, aOffset);
				} else {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%s\r\nRange: bytes=%ld-\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, aOffset);
				}
			}
			else
			{
				if(aPort != 80) {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%s:%d\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData, aPort);
				} else {
					sprintf(iRequset, "GET /%s HTTP/1.1\r\n%s\r\nConnection: keep-alive\r\n\r\n", iHostFileName, iHostMetaData);
				}
			}
		}
	}
...
TTInt CTTHttpClient::ParseResponseHeader(ITTStreamBufferingObserver* aObserver, TTUint32& aStatusCode)
{
	...
	LOGI("ParseResponseHeader return %d, %u", nErr, aStatusCode);
	return nErr;
}
...
TTInt CTTHttpClient::ConnectViaProxy(ITTStreamBufferingObserver* aObserver, const TTChar* aUrl, TTInt64 aOffset)
{
    //GKASSERT(aOffset >= 0);
    ...
    if (g_Domain != NULL) {
        //memset(&gProxyHostIP, 0, sizeof(TTIpAddr));
        if (gProxyHostIPV6 == NULL) {
            gProxyHostIPV6 = (TTIpAddr)malloc(sizeof(struct sockaddr_storage));
        }
        else{
            memset(gProxyHostIPV6, 0, sizeof(struct sockaddr_storage));
        }
        
        TTInt nErr = ResolveDNS(aObserver, g_Domain, gProxyHostIPV6);
        if( nErr != TTKErrNone)
        {
            return nErr;
        }
        
        nErr = ConnectServer(aObserver, gProxyHostIPV6, gProxyHostPort);
    }
    else
        nErr = ConnectServerIPV4Proxy(aObserver, gProxyHostIP, gProxyHostPort);
...
TTInt CTTHttpClient::Connect(ITTStreamBufferingObserver* aObserver, const TTChar* aUrl, TTInt64 aOffset)
{
	//GKASSERT(aOffset >= 0);
    ...
    if (iHostIP == NULL) {
        iHostIP = (TTIpAddr)malloc(sizeof(struct sockaddr_storage));
    }
    else{
        memset(iHostIP, 0, sizeof(struct sockaddr_storage));
    }
    mHostIP = 0;
...
TTInt CTTHttpClient::SendRequestAndParseResponse(_pFunConnect pFunConnect, ITTStreamBufferingObserver* aObserver, const TTChar* aUrl, TTInt aPort, TTInt64 aOffset)
{
	//send get file request
	LOGI("[Connected]: TT_Send_Request_C: offset = %lld", aOffset);
...
TTInt CTTHttpClient::Redirect(_pFunConnect pFunConnect, ITTStreamBufferingObserver* aObserver, TTInt64 aOffset)
{
...
TTInt CTTHttpClient::ResolveDNS(ITTStreamBufferingObserver* aObserver, TTChar* aHostAddr, TTIpAddr aHostIP)
{
    TTIpAddr * cachedHostIp = (TTIpAddr*)iDNSCache->get(aHostAddr);
    TTInt loopWaitCnt = 0;
    TTInt i;
    TTBool parseRet;
    
    if (cachedHostIp != IP_NOT_FOUND)
    {
    ...
    if (parseRet) {
        ...
        if (aHostIP->sa_family == AF_INET6) {
            ...
            if (inet_ntop(aHostIP->sa_family, numericAddress, pHostIP, INET6_ADDRSTRLEN) != NULL){
                mHostIP = inet_addr(pHostIP);
                if (strcmp(pHostIP, aHostAddr) != 0)
                {
                    iDNSCache->put(aHostAddr, (TTPtr)aHostIP, sizeof(struct sockaddr_storage));
                }
            }
        }
...
TTInt CTTHttpClient::ConnectServer(ITTStreamBufferingObserver* aObserver, const TTIpAddr aHostIP, TTInt& nPortNum)
{
    
	if((iSocketHandle = socket(aHostIP->sa_family, SOCK_STREAM, 0)) == KInvalidSocketHandler)//
	{
		LOGE("socket error");
	...
	SetSocketNonBlock(iSocketHandle); //aIP = ((struct sockaddr_in*)(res->ai_addr))->sin_addr;
...
#ifdef __TT_OS_IOS__
	TTInt nErr = connect(iSocketHandle, aHostIP, aHostIP->sa_len);
#else
    TTInt nErr = connect(iSocketHandle, aHostIP, sizeof(struct sockaddr));
#endif
...
TTInt CTTHttpClient::ConnectServerIPV4Proxy(ITTStreamBufferingObserver* aObserver, TTUint32 nHostIP, TTInt& nPortNum)
{
...
, iHostMetaData(NULL)
{
   ...
   iCancel = ETTFalse;
    ...
    //memset(&iHostIP, 0, sizeof(TTIpAddr));
    iHostIP = NULL;
    mHostIP = 0;
	iStatusCode = 0;
	iCancel = ETTFalse;
   ...
   // memset(&iSocketStore,0,sizeof(struct sockaddr_storage));
...
CTTHttpClient::~CTTHttpClient()
{
    ...
    if (iHostIP != NULL) {
        struct sockaddr_storage* tmp = (struct sockaddr_storage*)iHostIP;
        free(tmp);
        iHostIP = NULL;
    }

	SAFE_FREE(iHostMetaData);
}
		...
		*pEndofkey = '\0';
		...
		if (strncmp(iLineBuffer, aKey, strlen(aKey)) != 0)
		{
			if(bIsKeyChange){
				...
				if (strncmp(iLineBuffer, KContentType, strlen(KContentType)) == 0)
				{
					TTChar* pSrc = iLineBuffer + strlen(KContentType) + 1;
...
void CTTHttpClient::SetHostMetaData(TTChar* aHostHead)
{
	SAFE_FREE(iHostMetaData);
	if(aHostHead)
	{
		iHostMetaData = (TTChar*)malloc(strlen(aHostHead) + 1);
		strcpy(iHostMetaData, aHostHead);
	}
}
