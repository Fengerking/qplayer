...
public:
	...
	*/
	virtual	TTInt				ReadSync(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize);
	...
	* \return					文件字节数
	*/
	virtual	TTInt64				Size() const;

	virtual	TTUint64			BufferedSize();
	...
	* \param[in]	aReadSize	读取的字节数
	*/
	virtual	TTInt				ReadWait(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize);
	...
	*/
	virtual TTUint16			ReadUint16(TTInt64 aReadPos);// 按高字节在后面读
	...
	* \return					返回整数值，超过范围会Assert
	*/
	virtual TTUint16			ReadUint16BE(TTInt64 aReadPos);// 按高字节在前面读
	...
	* \return					返回整数值，超过范围会Assert
	*/
	virtual TTUint32			ReadUint32(TTInt64 aReadPos);
	...
	* \return					返回整数值，超过范围会Assert
	*/
	virtual TTUint32			ReadUint32BE(TTInt64 aReadPos);
	...
	* \return					返回整数值，超过范围会Assert
	*/
	virtual TTUint64			ReadUint64(TTInt64 aReadPos);
	...
	* \return					返回整数值，超过范围会Assert
	*/
	virtual TTUint64			ReadUint64BE(TTInt64 aReadPos);
...
private:
	...
	TTInt						CheckReadInt(TTInt64 aReadPos, TTInt aIntSize, TTInt& aIntOffset);//读取整数的验证是否超出范围或者将数据读入内存
	TTInt						CheckPreRead(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize);
	TTInt						Read(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize);
	void						PreRead(TTInt64 aReadPos);


private:

	FILE*						iFile;		/**< 操作文件的文件指针*/
	TTInt64						iFileSize;	/**<  文件大小*/
	...
	TTInt64						iCurPreReadBufferPos;/**< 预读Buffer数据在文件中的位置*/
};
#endif
