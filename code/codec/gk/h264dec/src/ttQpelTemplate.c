...
{                                                                       \
    ...
    for (i = 0; i < h; i++) {                                           \
        uint32_t a, b;                                                  \
        a = TTV_RN32(&src1[i * src_stride1]);                            \
        b = TTV_RN32(&src2[i * src_stride2]);                            \
        OP(*((uint32_t *) &dst[i * dst_stride]),                        \
           no_rnd_avg32(a, b));                                         \
        a = TTV_RN32(&src1[i * src_stride1 + 4]);                        \
        b = TTV_RN32(&src2[i * src_stride2 + 4]);                        \
...
{                                                                       \
    ...
    for (i = 0; i < h; i++) {                                           \
        ...
        a  = TTV_RN32(&src1[i * src_stride1]);                           \
        b  = TTV_RN32(&src2[i * src_stride2]);                           \
        c  = TTV_RN32(&src3[i * src_stride3]);                           \
        d  = TTV_RN32(&src4[i * src_stride4]);                           \
        l0 = (a & 0x03030303UL) +                                       \
             (b & 0x03030303UL) +                                       \
                  0x02020202UL;                                         \
        h0 = ((a & 0xFCFCFCFCUL) >> 2) +                                \
        ...
        a  = TTV_RN32(&src1[i * src_stride1 + 4]);                       \
        b  = TTV_RN32(&src2[i * src_stride2 + 4]);                       \
        c  = TTV_RN32(&src3[i * src_stride3 + 4]);                       \
        d  = TTV_RN32(&src4[i * src_stride4 + 4]);                       \
        l0 = (a & 0x03030303UL) +                                       \
             (b & 0x03030303UL) +                                       \
                  0x02020202UL;                                         \
        h0 = ((a & 0xFCFCFCFCUL) >> 2) +                                \
