...
void tt_h264_init_cabac_states(H264Context *h) {
    int i;
    const int8_t (*tab)[2];
    const int slice_qp = ttv_clip(h->qscale - 6*(h->sps.bit_depth_luma-8), 0, 51);

    if( h->slice_type_nos == TTV_PICTURE_TYPE_I ) tab = cabac_context_init_I;
...
static int decode_cabac_field_decoding_flag(H264Context *h) {
    const int mbb_xy = h->mb_xy - 2L*h->mb_stride;
...
//    if(!get_cabac(&h->cabac, &h->cabac_state[ctxbase+(amvd>2)+(amvd>32)])){
    ...
    if( mvd >= 9 ) {
        ...
        while( get_cabac_bypass( &h->cabac ) ) {
            ...
            k++;
            if(k>24){
                ttv_log(h->avctx, TTV_LOG_ERROR, "overflow in decode_cabac_mb_mvd\n");
                return INT_MIN;
            }
        }
        while( k-- ) {
...
static ttv_always_inline int get_cabac_cbf_ctx( H264Context *h, int cat, int idx, int max_coeff, int is_dc ) {
    int nza, nzb;
    int ctx = 0;
                               ...
                               int is_dc, int chroma422)
{
    ...
    static const int last_coett_flag_offset[2][14] = {
    ...
    static const int coett_abs_level_m1_offset[14] = {
        227+0, 227+10, 227+20, 227+30, 227+39, 426, 952+0, 952+10, 952+20, 708, 982+0, 982+10, 982+20, 766
    };
    static const uint8_t significant_coett_flag_offset_8x8[2][63] = {
    ...
    static const uint8_t sig_coett_offset_dc[7] = { 0, 0, 1, 1, 2, 2, 2 };
    /* node ctx: 0..3: abslevel1 (with abslevelgt1 == 0).
     * 4..7: abslevelgt1 + 3 (and abslevel1 doesn't matter).
     * map node ctx => cabac ctx for level=1 */
    static const uint8_t coett_abs_level1_ctx[8] = { 1, 2, 3, 4, 0, 0, 0, 0 };
    /* map node ctx => cabac ctx for level>1 */
    static const uint8_t coett_abs_levelgt1_ctx[2][8] = {
        { 5, 5, 5, 5, 6, 7, 8, 9 },
        { 5, 5, 5, 5, 6, 7, 8, 8 }, // 422/dc case
    };
    static const uint8_t coett_abs_level_transition[2][8] = {
    ...
    uint8_t *significant_coett_ctx_base;
    uint8_t *last_coett_ctx_base;
...
#define CC &h->cabac
    ...
    significant_coett_ctx_base = h->cabac_state
        + significant_coett_flag_offset[MB_FIELD(h)][cat];
    last_coett_ctx_base = h->cabac_state
        + last_coett_flag_offset[MB_FIELD(h)][cat];
    abs_level_m1_ctx_base = h->cabac_state
        + coett_abs_level_m1_offset[cat];

    if( !is_dc && max_coeff == 64 ) {
#define DECODE_SIGNIFICANCE( coefs, sig_off, last_off ) \
        for(last= 0; last < coefs; last++) { \
            uint8_t *sig_ctx = significant_coett_ctx_base + sig_off; \
            if( get_cabac( CC, sig_ctx )) { \
                uint8_t *last_ctx = last_coett_ctx_base + last_off; \
                index[coett_count++] = last; \
        ...
        const uint8_t *sig_off = significant_coett_flag_offset_8x8[MB_FIELD(h)];
#ifdef decode_significance
        coett_count = decode_significance_8x8(CC, significant_coett_ctx_base, index,
                                                 last_coett_ctx_base, sig_off);
    } else {
        if (is_dc && chroma422) { // dc 422
            DECODE_SIGNIFICANCE(7, sig_coett_offset_dc[last], sig_coett_offset_dc[last]);
        } else {
            coett_count = decode_significance(CC, max_coeff, significant_coett_ctx_base, index,
                                                 last_coett_ctx_base-significant_coett_ctx_base);
        }
#else
        DECODE_SIGNIFICANCE( 63, sig_off[last], tt_h264_last_coett_flag_offset_8x8[last] );
    } else {
        if (is_dc && chroma422) { // dc 422
            DECODE_SIGNIFICANCE(7, sig_coett_offset_dc[last], sig_coett_offset_dc[last]);
        } else {
    ...
    if( is_dc ) {
        ...
        h->non_zero_count_cache[scan8[n]] = coett_count;
    } else {
        if( max_coeff == 64 )
            fill_rectangle(&h->non_zero_count_cache[scan8[n]], 2, 2, 8, coett_count, 1);
        else {
            ttv_assert2( cat == 1 || cat ==  2 || cat ==  4 || cat == 7 || cat == 8 || cat == 11 || cat == 12 );
            h->non_zero_count_cache[scan8[n]] = coett_count;
        }
    }
...
#define STORE_BLOCK(type) \
    do { \
        uint8_t *ctx = coett_abs_level1_ctx[node_ctx] + abs_level_m1_ctx_base; \
 \
        int j= scantable[index[--coett_count]]; \
 \
        if( get_cabac( CC, ctx ) == 0 ) { \
            node_ctx = coett_abs_level_transition[0][node_ctx]; \
            if( is_dc ) { \
            ...
            int coett_abs = 2; \
            ctx = coett_abs_levelgt1_ctx[is_dc && chroma422][node_ctx] + abs_level_m1_ctx_base; \
            node_ctx = coett_abs_level_transition[1][node_ctx]; \
\
            while( coett_abs < 15 && get_cabac( CC, ctx ) ) { \
                coett_abs++; \
            } \
\
            if( coett_abs >= 15 ) { \
...
\
                coett_abs=1; \
                while( j-- ) { \
                    coett_abs += coett_abs + get_cabac_bypass( CC ); \
                } \
                coett_abs+= 14; \
            } \
\
            if( is_dc ) { \
                ((type*)block)[j] = get_cabac_bypass_sign( CC, -coett_abs ); \
            }else{ \
                ((type*)block)[j] = ((int)(get_cabac_bypass_sign( CC, -coett_abs ) * qmul[j] + 32)) >> 6; \
            } \
        } \
    } while ( coett_count );
...
static ttv_noinline void decode_cabac_residual_dc_internal(H264Context *h,
                                                          int16_t *block,
                                                          int cat, int n,
...
static ttv_noinline void decode_cabac_residual_dc_internal_422(H264Context *h,
                                                              int16_t *block,
                                                              int cat, int n,
                                                              const uint8_t *scantable,
                                                              int max_coeff)
...
static ttv_noinline void decode_cabac_residual_nondc_internal(H264Context *h,
                                                             int16_t *block,
                                                             int cat, int n,
                                                             const uint8_t *scantable,
                                                             const uint32_t *qmul,
...
static ttv_always_inline void decode_cabac_residual_dc(H264Context *h,
                                                      int16_t *block,
                                                      int cat, int n,
                                                      const uint8_t *scantable,
                                                      int max_coeff)
...
static ttv_always_inline void decode_cabac_residual_nondc(H264Context *h,
                                                         int16_t *block,
                                                         int cat, int n,
                                                         const uint8_t *scantable,
                                                         const uint32_t *qmul,
...
static ttv_always_inline void decode_cabac_luma_residual( H264Context *h, const uint8_t *scan, const uint8_t *scan8x8, int pixel_shift, int mb_type, int cbp, int p )
{
...
int tt_h264_decode_mb_cabac(H264Context *h) {
    ...
    if( h->slice_type_nos == TTV_PICTURE_TYPE_B ) {
        int ctx = 0;
        ttv_assert2(h->slice_type_nos == TTV_PICTURE_TYPE_B);
...
decode_intra_mb:
    ...
    if(IS_INTRA_PCM(mb_type)) {
        ...
        // decode, I think it should be done by a function in ttCabac.h (or cabac.c).
        ...
        tt_init_cabac_decoder(&h->cabac, ptr, h->cabac.bytestream_end - ptr);
    ...
    if( IS_INTRA( mb_type ) ) {
        ...
        if( IS_INTRA4x4( mb_type ) ) {
            if( dct8x8_allowed && get_cabac_noinline( &h->cabac, &h->cabac_state[399 + h->neighbor_transform_size] ) ) {
                ...
                for( i = 0; i < 16; i++ ) {
                    ...
                    ttv_dlog(h->avctx, "i4x4 pred=%d mode=%d\n", pred,
                            h->intra4x4_pred_mode_cache[scan8[i]]);
                }
            }
            write_back_intra_pred_mode(h);
            if( tt_h264_check_intra4x4_pred_mode(h) < 0 ) return -1;
        } else {
        ...
        if(decode_chroma){
            ...
            pred_mode= tt_h264_check_intra_pred_mode( h, pred_mode, 1 );
                    ...
                    if(IS_DIR(h->sub_mb_type[i], 0, list)){
                        if (local_ref_count[list] > 1) {
                            ...
                            if (ref[list][i] >= (unsigned)local_ref_count[list]) {
                                ttv_log(h->avctx, TTV_LOG_ERROR, "Reference %d >= %d\n", ref[list][i], local_ref_count[list]);
                                return -1;
                            }
                        }else
                            ref[list][i] = 0;
        ...
        if(get_cabac_noinline( &h->cabac, &h->cabac_state[60 + (h->last_qscale_diff != 0)])){
            ...
            while( get_cabac_noinline( &h->cabac, &h->cabac_state[60 + ctx] ) ) {
                ...
                if(val > 2*max_qp){ //prevent infinite loop
                    ttv_log(h->avctx, TTV_LOG_ERROR, "cabac decode of qscale diff failed at %d %d\n", h->mb_x, h->mb_y);
                    return -1;
                }
            }
