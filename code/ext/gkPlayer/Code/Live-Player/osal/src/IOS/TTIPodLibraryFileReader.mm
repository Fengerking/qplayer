...
TTInt CTTIPodLibraryFileReader::ReadSync(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{        
    ...
    else
    {
        ...
        else if (pAssetReader.status == AVAssetReaderStatusFailed)
        {
            ...
            if (pAssetReader.error.code == AVErrorOperationInterrupted || (![[GKUIDeviceHardware instance] isSystemVersionLargeEqual6] && pAssetReader.error.code == AVErrorUnknown)) {
...
TTInt64 CTTIPodLibraryFileReader::Size() const
{
	return iDuration;
}

TTInt CTTIPodLibraryFileReader::ReadWait(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
    GKASSERT(ETTFalse);
}
...
TTInt CTTIPodLibraryFileReader::PrepareCache(TTInt64 aReadPos, TTInt aReadSize, TTInt aFlag)
{
    return 0;
}
