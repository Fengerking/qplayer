...
TTInt CTTHttpReaderProxy::Open(const TTChar* aUrl)
{
 ...
 reconnect:
	...
	else
	{
		...
		iPrefetchSize = (GKNetWorkConfig::getInstance()->getActiveNetWorkType() == EActiveNetWorkWIFI) ? KWIFI_PRE_FETCH_SIZE : KGPRS_PRE_FETCH_SIZE;
...
TTBool CTTHttpReaderProxy::ProcessBufferingIssue(TTInt aDesireBufferedPos, TTInt aDesireBufferedSize)
{
	...
	TTInt nPrefetchSize = (GKNetWorkConfig::getInstance()->getActiveNetWorkType() == EActiveNetWorkWIFI) ? KWIFI_PRE_FETCH_SIZE : KGPRS_PRE_FETCH_SIZE;
...
TTInt CTTHttpReaderProxy::Read(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
	GKASSERT(iHttpCacheFile != NULL);
...
TTInt CTTHttpReaderProxy::ReadWait(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
	GKASSERT(iHttpCacheFile != NULL);
...
TTInt64 CTTHttpReaderProxy::Size() const
{
...
TTInt CTTHttpReaderProxy::ReConnectServer()
{
    ...
    {
        iHttpClient->Disconnect();
        TTInt64 nOffset = iHttpCacheFile->CachedSize();
...
TTInt64 CTTHttpReaderProxy::BufferedSize()
{
	TTInt64 nBufferedSize = 0;
...
TTInt CTTHttpReaderProxy::PrepareCache(TTInt64 aDesireBufferedPos, TTInt aDesireBufferedSize, TTInt aFlag)
{
	TTInt nWaitCount = 0;
	TTBool bIsBuffering = IsDesiredDataBuffering(aDesireBufferedPos, aDesireBufferedSize);
