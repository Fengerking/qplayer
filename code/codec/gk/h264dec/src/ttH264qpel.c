...
ttv_cold void tt_h264qpel_init(H264QpelContext *c, int bit_depth)
{
#undef FUNCC
...
#if ARCH_AARCH64
    if (ARCH_AARCH64)
        tt_h264qpel_init_aarch64(c, bit_depth);
#endif
#if ARCH_ARM
    if (ARCH_ARM)
        tt_h264qpel_init_arm(c, bit_depth);
#endif
#if ARCH_PPC
    if (ARCH_PPC)
        tt_h264qpel_init_ppc(c, bit_depth);
#endif
#if ARCH_X86
    if (ARCH_X86)
        tt_h264qpel_init_x86(c, bit_depth);
#endif
}
