...
static int get_scale_factor(H264Context *const h, int poc, int poc1, int i)
{
    ...
    if (td == 0 || h->ref_list[0][i].long_ref) {
        ...
        int tb = ttv_clip(poc - poc0, -128, 127);
        int tx = (16384 + (FFABS(td) >> 1)) / td;
        return ttv_clip((tb * tx + 32) >> 6, -1024, 1023);
    }
}
                                   ...
                                   int mb_y)
{
    ...
    if (!HAVE_THREADS || !(h->avctx->active_thread_type & TT_THREAD_FRAME))
        return;

    /* FIXME: It can be safe to access mb stuff
     * even if pixels aren't deblocked yet. */

    tt_thread_await_progress(&ref->tf,
...
#define MB_TYPE_16x16_OR_INTRA (MB_TYPE_16x16 | MB_TYPE_INTRA4x4 | \
    ...
    for (list = 0; list < 2; list++) {
        ...
        if (ref[list] >= 0) {
            ...
            ttv_assert2(ref[list] < (h->ref_count[list] << !!FRAME_MBAFF(h)));
        } else {
            ...
            if (!IS_INTRA(mb_type_col[0]) && !h->ref_list[1][0].long_ref &&
                ...
                if (IS_SUB_8X8(sub_mb_type)) {
                    ...
                    for (i4 = 0; i4 < 4; i4++) {
                        ...
                        if (FFABS(mv_col[0]) <= 1 && FFABS(mv_col[1]) <= 1) {
                            if (ref[0] == 0)
                                TTV_ZERO32(h->mv_cache[0][scan8[i8 * 4 + i4]]);
                            if (ref[1] == 0)
                                TTV_ZERO32(h->mv_cache[1][scan8[i8 * 4 + i4]]);
                            m++;
                        }
                    }
                    if (!(m & 3))
            ...
            for (i8 = 0; i8 < 4; i8++) {
                ...
                if (IS_SUB_8X8(sub_mb_type)) {
                    ...
                    for (i4 = 0; i4 < 4; i4++) {
                        ...
                        mv_l0[0] = (scale * mv_col[0] + 128) >> 8;
                        mv_l0[1] = (scale * mv_col[1] + 128) >> 8;
                        TTV_WN32A(h->mv_cache[1][scan8[i8 * 4 + i4]],
