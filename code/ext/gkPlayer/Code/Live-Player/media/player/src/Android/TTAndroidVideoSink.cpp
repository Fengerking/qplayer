...
CTTAndroidVideoSink::CTTAndroidVideoSink(CTTSrcDemux* aSrcMux, TTCBaseAudioSink* aAudioSink, GKDecoderType aDecoderType)
...
TTInt CTTAndroidVideoSink::setView(void * pView)
{
	...
	if(mView != NULL) {
		mSurfaceObj = pEnv->NewGlobalRef(*((jobject *)mView));
	}
...
TTInt CTTAndroidVideoSink::newVideoView()
{
	...
	mNativeWnd = (ANativeWindow *)mpANativeWindow_fromSurface(env.getEnv(), (jobject)mSurfaceObj);
	if(mNativeWnd == NULL) {
		return TTKErrNotReady;
	}
...
void CTTAndroidVideoSink::checkHWEnable(unsigned int nCodecType) 
{
	TTChar szProp[64];
	memset (szProp, 0, 64);
	...
	if(strstr (szProp, "4.1") == szProp || strstr (szProp, "4.2") == szProp || strstr (szProp, "4.3") == szProp || strstr (szProp, "4.4") == szProp || strstr (szProp, "5.") == szProp ||
		strstr (szProp, "6.") == szProp || strstr (szProp, "7.") == szProp) {
	...
	if(mVideoFormat.Width*mVideoFormat.Height < 640*480 && strstr(szProp, "SM-") != szProp) {
		mHWDec = TT_VIDEODEC_SOFTWARE;
		checkYUVEnable(0);
	}
...
#endif	
	if(mCPUNum > 4) {
		LOGI("CTTAndroidVideoSink::checkCPUFeature mCPUNum %d, Actural use %d", mCPUNum, 4);
		mCPUNum = 4;
	} else {
...
void CTTAndroidVideoSink::videoTrack_setSurface()
{
	...
	pEnv->CallIntMethod(mVideoTrackObjRef, mJMethodSetSurface, mSurfaceObj);
}
