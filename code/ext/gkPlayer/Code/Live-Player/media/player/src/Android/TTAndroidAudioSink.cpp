...
* Author : yongping.lin
* Copyright : Copyright (c) 2011 GoKu Software Ltd. All rights reserved.
* Description : CTTAndroidAudioSink µœ÷
*/
...
TTInt CTTAndroidAudioSink::render()
{
	...
	if (nErr == TTKErrNone){
		if(mSinkBuffer.nFlag & TT_FLAG_BUFFER_EOS) {
			...
			audioFormatChanged();
			startOne(-1);
		} else if(mSinkBuffer.nSize > 0 && mSinkBuffer.nSize < mMaxPCMCount){
