/**
* File : TTMediaParser.cpp
 * Description : CTTMediaParserþ
*/
...
TTInt CTTMediaParser::SeekWithoutFrmPosTab(TTInt aStreamId, TTInt aFrmIdx, TTMediaFrameInfo& aFrameInfo)
{
	...
	if (aFrmIdx == (iPreSeekIdx + 1))//Èç¹û·ÃÎÊÏÂÒ»Ö¡
	{
	...
	else if ((aFrmIdx == iPreSeekIdx) && (aFrmIdx != 0))//Èç¹ûÉÏ´ÎseekºóÃ»ÓÐ·ÖÅäµ½buf£¬ÔòÐèÒªÔÙseekÍ¬Ò»Ö¡
	{
...
{
	if(aType == TT_PID_COMMON_DELAYSTATUS) {
		if(aParam) {
			*((TTInt *)aParam) = iDelayStatus;
		}
		return TTKErrNone;
	}
	return TTKErrNotSupported;
	...
	case TTKErrEof:	
		{
			...
			if (nFrameSize > 0)	{
				...
				else
				{
					...
					if (nErr != TTKErrEof && (iDataReader.Id() == ITTDataReader::ETTDataReaderIdHttp || iDataReader.Id() == ITTDataReader::ETTDataReaderIdBuffer)) {
...
TTInt CTTMediaParser::ConvertAVCFrame(TTPBYTE pFrame, TTUint32 nSize, TTUint32& nFrameLen, TTInt& IsKeyFrame)	
{
	...
	while (pBuffer - pFrame + iNALLengthSize < nSize)
	{
		...
		{
            nNalLen = leftSize;
            IsKeyFrame |= TT_FLAG_BUFFER_FLUSH;
            nNalType = 1;
