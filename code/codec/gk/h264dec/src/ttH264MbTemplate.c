...
#if SIMPLE
#   define FUNC(n) TTV_JOIN(n ## _simple_, BITS)
...
#undef  CHROMA_IDC
#define CHROMA_IDC 1
#include "ttH264McTemplate.c"

#undef  CHROMA_IDC
#define CHROMA_IDC 2
#include "ttH264McTemplate.c"

static ttv_noinline void FUNC(hl_decode_mb)(H264Context *h)
{
    ...
    const int is_h264 = !CONFIG_SVQ3_DECODER || SIMPLE || h->avctx->codec_id == TTV_CODEC_ID_H264;
    ...
    if (!SIMPLE && IS_INTRA_PCM(mb_type)) {
        ...
        if (PIXEL_SHIFT) {
            ...
            init_get_bits(&gb, h->intra_pcm_ptr,
                          tt_h264_mb_sizes[h->sps.chroma_format_idc] * bit_depth);
                    ...
                    for (j = 1; j < 3; j++) {
                        for (i = j * 16; i < j * 16 + 4; i++)
                            if (h->non_zero_count_cache[scan8[i]] || h->mb[i * 16]) {
                                ...
                                tt_svq3_add_idct_c(ptr, h->mb + i * 16,
                                                   uvlinesize,
                                                   tt_h264_chroma_qp[0][h->qscale + 12] - 12, 2);
                            }
                    }
                }
#endif
...
static ttv_noinline void FUNC(hl_decode_mb_444)(H264Context *h)
{
    const int mb_x    = h->mb_x;
    const int mb_y    = h->mb_y;
    const int mb_xy   = h->mb_xy;
