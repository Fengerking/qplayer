/**
* File : TTMediaPlayer.cpp  
* Created on : 2014-9-1
* Author : yongping.lin
* Description : GKMediaPlayer实现文件
*/
...
CGKMediaPlayer::CGKMediaPlayer(IGKMediaPlayerObserver* aPlayerObserver, const TTChar* aPluginPath)
:mUrl(NULL)
...
CGKMediaPlayer::~CGKMediaPlayer()
{
	doStop(false);
	//TTInt64 nStart = GetTimeOfDay();
	mCritical.Lock();
	SAFE_DELETE(mVideoSink);
	...
	GKNetWorkConfig::release();
#ifdef __MEDIA_PLAYER__
	CTTAudioEffectManager::Release();
#endif
	mCriStatus.Destroy();
	mCriEvent.Destroy();
	mCritical.Destroy();
...
const TTChar* CGKMediaPlayer::Url()
{
	return mUrl;
}

int CGKMediaPlayer::IsLimited()
{
    time_t rawtime;
    struct tm * timeinfo;
    time(&rawtime);
    ...
    if((timeinfo->tm_year <= 116 && timeinfo->tm_mon <= 11) || (timeinfo->tm_year <= 117 && timeinfo->tm_mon <= 1)) {
        return 0;
    }
...
TTInt CGKMediaPlayer::SetDataSourceAsync(const TTChar* aUrl, TTInt nFlag)
{
	if(aUrl == NULL)
...
//    }
	...
	//LOGI("++++SetDataSourceAsync: Time %lld", GetTimeOfDay());
  	if(nFlag&ESourceChanged) {
		...
		GKCAutoLock LockEvent(&mCriEvent);
		if(mHandleThread) {
...
TTInt CGKMediaPlayer::SetDataSourceSync(const TTChar* aUrl, TTInt nFlag)
{
	if(aUrl == NULL)
		return TTKErrArgument;
...
TTInt CGKMediaPlayer::SetDataSource()
{
	...
	TTInt nAVFlag = 0;
	mCritical.Lock();
	if((mAudioSink == 0 && mVideoSink == 0) || bStop) {
		mSrcDemux->RemoveDataSource();
		mCritical.UnLock();
...
void CGKMediaPlayer::InitSink()
{
	GKCAutoLock Lock(&mCritical);

	TTBool doAudioEffect = true;
	...
	const RGKPointerArray<TTAudioInfo>& rMediaAudioArrary = tMeidaInfo.iAudioInfoArray;
	...
	if(rMediaAudioArrary.Count() > 0) {
		...
		if(nErr != TTKErrNone) {
			SAFE_DELETE(mAudioSink);
            postMsgEvent(-1, ENotifyAudioFormatUpsupport, nErr, 0, NULL);
		}
	} else {
		SAFE_DELETE(mAudioSink);
	}
	...
	if(tMeidaInfo.iVideoInfo) {
		mVideoStreamId = tMeidaInfo.iVideoInfo->iStreamId;

		mSrcDemux->SelectStream(EMediaTypeVideo, mVideoStreamId);

		GKCAutoLock videoLock(&mCriVideo);

		if(mVideoSink == NULL) {

#ifdef __TT_OS_ANDROID__
			mVideoSink = new CTTAndroidVideoSink(mSrcDemux, mAudioSink, mDecoderType);
			((CTTAndroidVideoSink *)mVideoSink)->setJniInfo(mpVideotrackJClass);
#endif //_LINUX_ANDROID
#ifdef __TT_OS_WINDOWS__
			mVideoSink = new CTTWinVideoSink(mSrcDemux, mAudioSink, mDecoderType);
#endif // _WIN32
#ifdef __TT_OS_IOS__
            mVideoSink = new CTTIosVideoSink(mSrcDemux, mAudioSink, mDecoderType);
#endif // _IOS
		}
        mVideoSink->setMotionEnable(mMotionEnable);
        mVideoSink->setTouchEnable(mTouchEnable);
        mVideoSink->setRendType(mRenderType);
		mVideoSink->setObserver(&mVideoObserver);
        mVideoSink->setAudioSink(mAudioSink);
		mVideoSink->setView(mView);
		mVideoSink->setParam(TT_PID_COMMON_DATASOURCE, mSrcDemux);


		nErr = mVideoSink->open(tMeidaInfo.iVideoInfo);
		if(nErr != TTKErrNone) {
			SAFE_DELETE(mVideoSink);
            postMsgEvent(-1, ENotifyVideoFormatUpsupport, nErr, 0, NULL);
		} else {
			doAudioEffect = false;
		}
	} else {
	...
	if(mAudioSink)
		mAudioSink->setParam(TT_PID_AUDIO_DOEFFECT, &doAudioEffect);

}

void CGKMediaPlayer::Pause(TTBool aFadeOut)
{
	TTInt nErr = TTKErrNone;
	GKPlayStatus tStatus = GetPlayStatus();
...
void CGKMediaPlayer::Resume(TTBool aFadeIn)
{
	...
	if(mAudioSink) {
		if(mVideoSink) {
			if(bSeeking == true) {
				nErr = mAudioSink->resume(true);
			} else {
				nErr = mAudioSink->resume();
			}
			nErr = mVideoSink->resume();
		} else {
...
TTInt CGKMediaPlayer::doStop(TTBool aIntra)
{
...
TTInt CGKMediaPlayer::Stop(TTBool aSync)
{
...
#ifdef __TT_OS_IOS__
    if (EStatusStoped != GetPlayStatus()) {
        if(mVideoSink) {
            mVideoSink->setView(NULL);
        }
    }
#endif
...
void CGKMediaPlayer::updateView()
{
#ifdef __TT_OS_ANDROID__
	CJniEnvUtil	env(gJVM);
	JNIEnv* pEnv = env.getEnv();
	...
	mSurfaceObj = pEnv->NewGlobalRef(g_Surface);

	mView = NULL;
	if(mSurfaceObj) {
		mView = (void *)&mSurfaceObj;
	}
#endif
}

void CGKMediaPlayer::SetView(void* aView)
{
	GKCAutoLock videoLock(&mCriVideo);
	mView = aView;
	updateView();
	if(mVideoSink) {
		mVideoSink->setView(aView);
	}
}

void CGKMediaPlayer::SetVolume(TTInt aLVolume, TTInt aRVolume)
{
	GKPlayStatus tPlayStatus = GetPlayStatus();
...
TTInt CGKMediaPlayer::GetVolume()
{
	TTInt nVolume = 0;
	GKPlayStatus tPlayStatus = GetPlayStatus();
	if ((tPlayStatus == EStatusPlaying) || (tPlayStatus == EStatusPaused) || (tPlayStatus == EStatusPrepared))
...
TTInt64 CGKMediaPlayer::seek(TTInt64 aPosition, TTInt aOption)
{
	GKPlayStatus tStatus = GetPlayStatus();
	...
	if(aPosition >= 0) {
		...
		if (mVideoSink != NULL) {
			mVideoSink->syncPosition(aPosition, aOption);
		}
	}
	...
	if(tStatus == EStatusPlaying) {
		Resume();
	} else if(tStatus == EStatusPaused && mVideoSink != NULL && aPosition >= 0) {
		mVideoSink->startOne(-1);
	}
...
TTInt64 CGKMediaPlayer::SetPosition(TTInt64 aPosition, TTInt aOption)
{
	mCriStatus.Lock();
...
void CGKMediaPlayer::setSeekStatus(TTBool bSeeking)
{
	GKCAutoLock Lock(&mCriStatus);
	mSeekable = bSeeking;
	mSeeking = bSeeking;
...
TTBool CGKMediaPlayer::getSeekable()
{
...
TTUint64 CGKMediaPlayer::GetPosition()
{
	TTUint64 nPos = 0;
	mCriStatus.Lock();
	...
	GKPlayStatus tPlayStatus = GetPlayStatus();
	if ((tPlayStatus == EStatusStarting) || (tPlayStatus == EStatusStoped) || (tPlayStatus ==EStatusPrepared)) {
		if(nPrePlayPos)
			nPos = nPrePlayPos;
		return nPos;
	}
...
TTInt64 CGKMediaPlayer::GetPlayTime()
{
   	TTUint64 nPos = 0;
	...
	if (mVideoSink != NULL)	{
		nPos = mVideoSink->getPlayTime();
		return nPos;
	}
	return 0;
}

TTInt CGKMediaPlayer::onNotifyEvent(TTInt nMsg, TTInt nVar1, TTInt nVar2, void* nVar3)
{
	if(nMsg == ENotifyBufferingStart) {
		...
		} else if(mVideoSink != NULL) {
			mVideoSink->pause();
			mVideoSink->setBufferStatus(ETTBufferingStart);
		}
		mCritical.UnLock();
		...
		if (mAudioSink != NULL)	{
			...
			mVideoSink->setBufferStatus(ETTBufferingDone);
			if(GetPlayStatus() == EStatusPlaying)
				mVideoSink->resume();
		}
		mCritical.UnLock();
	...
	if(mPlayerObserver)
		mPlayerObserver->PlayerNotifyEvent((GKNotifyMsg)nMsg, nVar1, nVar2, (TTChar *)nVar3);

	return TTKErrNone;
}


TTUint CGKMediaPlayer::Duration()
{	
	TTUint nDuration = 0;
...
TTUint CGKMediaPlayer::BufferedSize()
{
...
TTUint CGKMediaPlayer::BandPercent()
{
...
TTUint CGKMediaPlayer::BandWidth()
{
	TTInt nBandWidth = 0;
	if(mSrcDemux != NULL)	{
...
TTInt CGKMediaPlayer::BufferedPercent(TTInt& aBufferedPercent)
{	
...
TTInt CGKMediaPlayer::GetCurrentFreqAndWave(TTInt16 *aFreq, TTInt16 *aWave, TTInt aSampleNum)
{
#ifdef __MEDIA_PLAYER__
...
TTInt CGKMediaPlayer::SetParam(TTInt nID, void* aParam)
{
	return TTKErrNone;
}

TTInt CGKMediaPlayer::GetParam(TTInt nID, void* aParam)
{
	return TTKErrNone;
}

void CGKMediaPlayer::SetActiveNetWorkType(GKActiveNetWorkType aNetWorkType)
{
	LOGI("CTTMediaPlayer::SetActiveNetWorkType: %d", aNetWorkType);
	GKNetWorkConfig* pNetWorkConfig = GKNetWorkConfig::getInstance();
	pNetWorkConfig->SetActiveNetWorkType(aNetWorkType);
	LOGI("CTTMediaPlayer::SetActiveNetWorkType return");
}

void CGKMediaPlayer::SetNetWorkProxy(TTBool aNetWorkProxy)
{
	GKPlayStatus tPlayStatus = GetPlayStatus();
...
void CGKMediaPlayer::SetDecoderType(GKDecoderType aDecoderType)
{
	...
	if ((tPlayStatus == EStatusPlaying) || (tPlayStatus == EStatusPaused) || (tPlayStatus == EStatusPrepared))
	{
		mCritical.Lock();
		if(mVideoSink) {
			mVideoSink->setDecoderType(aDecoderType);
		}
		mCritical.UnLock();
	}	
}
...
#ifdef __TT_OS_IOS__
void CGKMediaPlayer::SetBalanceChannel(float aVolume)
{
    if (mAudioSink != NULL) {
...
void CGKMediaPlayer::SetRotate()
{
    if (mVideoSink != NULL) {
        mVideoSink->SetRotate();
    }
}

void CGKMediaPlayer::SetVolume(float aVolume)
{
    if (mAudioSink != NULL) {
...
TTBool CGKMediaPlayer::IsLiveMode()
{
    return mSrcDemux->IsLiveMode();
}

TTInt CGKMediaPlayer::GetAVPlayType()
{
    return mAVTypeFlag;
}
#endif

void CGKMediaPlayer::SetRendType(TTInt aRenderType)
{
    mRenderType = aRenderType;
    mCritical.Lock();
    if (mVideoSink != NULL) {
        mVideoSink->setRendType(aRenderType);
    }
    mCritical.UnLock();
    
}

void CGKMediaPlayer::SetMotionEnable(bool aEnable)
{
    mMotionEnable = aEnable;
    mCritical.Lock();
    if (mVideoSink != NULL) {
        mVideoSink->setMotionEnable(aEnable);
    }
    mCritical.UnLock();
}

void CGKMediaPlayer::setTouchEnable(bool aEnable)
{
    mTouchEnable = aEnable;
    mCritical.Lock();
    if (mVideoSink != NULL) {
        mVideoSink->setTouchEnable(aEnable);
    }
    mCritical.UnLock();
}

TTInt CGKMediaPlayer::Play()
{
	...
	if(mAudioSink) {
		if(mVideoSink) {
			nErr = mAudioSink->start(bPause, true);
			if(nErr == TTKErrNone) {
				nErr = mVideoSink->start(bPause);
			}
		} else {
...
void CGKMediaPlayer::SetPlayRange(TTUint aStartTime, TTUint aEndTime)
{
	GKPlayStatus tStatus = GetPlayStatus();
	if(tStatus == EStatusStoped || tStatus == EStatusStarting)
		return;
	...
	if(mVideoSink) {
		mVideoSink->setPlayRange(aStartTime, aEndTime);
	}

	seek(aStartTime, 0);
}

void CGKMediaPlayer::SetCacheFilePath(const TTChar* aCacheFilePath)
{
...
void CGKMediaPlayer::SaveAudioTrackJClass(void* pJclass)
{
	mpAudiotrackJClass = pJclass;
}

void* CGKMediaPlayer::GetAudiotrackClass()
{
	return mpAudiotrackJClass;
}

void CGKMediaPlayer::SetMaxOutPutSamplerate(TTInt aSampleRate)
{
	gMaxOutPutSamplerate = aSampleRate;
}

void CGKMediaPlayer::SaveVideoTrackJClass(void* pJclass)
{
	mpVideotrackJClass = pJclass;
}

void* CGKMediaPlayer::GetVideotrackClass()
{
	return mpVideotrackJClass;
}
#endif

TTInt CGKMediaPlayer::onPreSource(TTInt nMsg, TTInt nVar1, TTInt nVar2, void* nVar3)
{
...
TTInt CGKMediaPlayer::onSCEvent(TTInt nMsg, TTInt nVar1, TTInt nVar2, void* nVar3)
{
	TTInt nErr = TTKErrNotFound;
	...
	if(nMsg == ENotifySCMediaStartToOpen) {
		...
		GKPlayStatus nStatus = GetPlayStatus();
		...
		if(mAudioSink) {
			if(mVideoSink) {
				...
				mVideoSink->syncPosition(nPlayingTime);
				mVideoSink->start(bPause);
			} else {
				mAudioSink->syncPosition(nPlayingTime);
				mAudioSink->start(bPause, false);
			}		
		...
		if(mAudioSink) {
			if(mVideoSink) {
				...
				mAudioSink->start(bPause, false);
				mVideoSink->syncPosition(nPlayingTime, nOption);
				mVideoSink->start(bPause);
			} else {
				mAudioSink->syncPosition(nPlayingTime, nOption);
				mAudioSink->start(bPause, false);
...
TTInt CGKMediaPlayer::onSetDataSource(TTInt nMsg, TTInt nVar1, TTInt nVar2, void* nVar3)
{
	TTInt nErr = SetDataSource();
	return nErr;
}

TTInt CGKMediaPlayer::onStop(TTInt nMsg, TTInt nVar1, TTInt nVar2, void* nVar3)
{
	TTInt nErr = doStop(false);
	return nErr;
}

TTInt CGKMediaPlayer::postSetDataSourceEvent (TTInt  nDelayTime)
{
	...
	if (pEvent == NULL)
		pEvent = new TTCMediaPlayerEvent (this, &CGKMediaPlayer::onSetDataSource, EEventLoad);
	mHandleThread->postEventWithDelayTime (pEvent, nDelayTime);

	return TTKErrNone;
}

TTInt CGKMediaPlayer::postStopEvent(TTInt  nDelayTime)
{
	if (mHandleThread == NULL)
		return TTKErrNotFound;
	...
	if (pEvent == NULL)
		pEvent = new TTCMediaPlayerEvent (this, &CGKMediaPlayer::onStop, EEventClose);
	mHandleThread->postEventWithDelayTime (pEvent, nDelayTime);

	return TTKErrNone;
}

TTInt CGKMediaPlayer::postPreSrcEvent (TTInt  nDelayTime, TTInt32 nID, int nParam1, int nParam2, void * pParam3)
{
	if (mHandleThread == NULL)
		return TTKErrNotFound;
	...
	if (pEvent == NULL)
		pEvent = new TTCMediaPlayerEvent (this, &CGKMediaPlayer::onPreSource, EEventMsg, nID, nParam1, nParam2, pParam3);
	else
...
TTInt CGKMediaPlayer::postSCMsgEvent (TTInt  nDelayTime, TTInt32 nID, int nParam1, int nParam2, void * pParam3)
{
	if (mHandleThread == NULL)
		return TTKErrNotFound;
	...
	TTBaseEventItem * pEvent = mHandleThread->getEventByType(EEventMsg);
	if (pEvent == NULL)
		pEvent = new TTCMediaPlayerEvent (this, &CGKMediaPlayer::onSCEvent, EEventMsg, nID, nParam1, nParam2, pParam3);
	else
		pEvent->setEventMsg(nID, nParam1, nParam2, pParam3);
	mHandleThread->postEventWithDelayTime (pEvent, nDelayTime);
...
TTInt CGKMediaPlayer::postMsgEvent (TTInt nDelayTime, TTInt32 nID, int nParam1, int nParam2, void * pParam3)
{
	if (mMsgThread == NULL)
		return TTKErrNotFound;
	...
	if (pEvent == NULL)
		pEvent = new TTCMediaPlayerEvent (this, &CGKMediaPlayer::onNotifyEvent, EEventMsg, nID, nParam1, nParam2, pParam3);
	else
		pEvent->setEventMsg(nID, nParam1, nParam2, pParam3);
...
TTInt CGKMediaPlayer::SetPlayStatus(GKPlayStatus aStatus)
{
...
GKPlayStatus CGKMediaPlayer::GetPlayStatus()
{
	GKCAutoLock Lock(&mCriStatus);
	GKPlayStatus tStatus = mPlayStatus;

	return tStatus;
}

TTInt CGKMediaPlayer::ttSrcCallBack (void* pUserData, TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	CGKMediaPlayer* pMediaPlayer = (CGKMediaPlayer*)pUserData;
...
TTInt CGKMediaPlayer::ttPreSrcCallBack (void* pUserData, TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	CGKMediaPlayer* pMediaPlayer = (CGKMediaPlayer*)pUserData;
...
TTInt CGKMediaPlayer::handlePreSrcMsg (TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
...
TTInt CGKMediaPlayer::ttSCSrcCallBack (void* pUserData, TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	CGKMediaPlayer* pMediaPlayer = (CGKMediaPlayer*)pUserData;
...
TTInt CGKMediaPlayer::handleSCSrcMsg (TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
...
TTInt CGKMediaPlayer::handleSrcMsg (TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
...
TTInt CGKMediaPlayer::ttAudioCallBack (void* pUserData, TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	CGKMediaPlayer* pMediaPlayer = (CGKMediaPlayer*)pUserData;
...
TTInt CGKMediaPlayer::handleAudioMsg (TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	GKCAutoLock Lock(&mCriEvent);

	if(nID == ENotifyComplete) {
		if(mVideoSink == NULL || mVideoSink->isEOS()) {
			mCriEvent.Lock();
		...
		if(mVideoSink != NULL) {
			TTBool bReset = true;
			mVideoSink->setParam(TT_PID_VIDEO_TIMERESET, &bReset);
		}
		return TTKErrNone;
...
TTInt CGKMediaPlayer::ttVideoCallBack (void* pUserData, TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	CGKMediaPlayer* pMediaPlayer = (CGKMediaPlayer*)pUserData;
...
TTInt CGKMediaPlayer::handleVideoMsg (TTInt32 nID, TTInt32 nParam1, TTInt32 nParam2, void * pParam3)
{
	GKCAutoLock Lock(&mCriEvent);
...
#ifdef __MEDIA_PLAYER__
TTInt CGKMediaPlayer::Decode(const TTChar* aUrl,TTInt size)
{
	return CTTPureDecodeEntity::Instance()->Decode(aUrl, size);
	//return 0;
}

TTUint8* CGKMediaPlayer::GetPCMData()
{
	return CTTPureDecodeEntity::Instance()->GetPCMData();
	//return NULL;
}

TTInt CGKMediaPlayer::GetPCMDataSize()
{
	return CTTPureDecodeEntity::Instance()->GetPCMDataSize();
	//return 0;
}

TTInt CGKMediaPlayer::GetPCMDataChannle()
{
	return CTTPureDecodeEntity::Instance()->GetChannels();
	//return 0;
}

TTInt CGKMediaPlayer::GetPCMDataSamplerate()
{
	return CTTPureDecodeEntity::Instance()->GetSamplerate();
	//return 0;
}

void CGKMediaPlayer::CancelGetPCM()
{
	CTTPureDecodeEntity::Instance()->Cancel();
}

void CGKMediaPlayer::SetTimeRange(TTInt start, TTInt end, TTInt decodeStartPos)
{
	CTTPureDecodeEntity::Instance()->SetTimeRange(start, end, decodeStartPos);
}

TTInt CGKMediaPlayer::DecodeEntireFile(const TTChar* aUrl, const TTChar* OutPutPath)
{
	return CTTPureDecodeEntity::Instance()->Decode(aUrl, OutPutPath);
}

TTInt CGKMediaPlayer::GetDataFromSpecifiedTimePos(const TTChar* aUrl, TTInt timePos, TTInt duration)
{
	return CTTPureDecodeEntity::Instance()->GetDataFromSpecifiedTimePos(aUrl, timePos ,duration);
}
#endif
