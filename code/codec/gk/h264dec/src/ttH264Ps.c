...
#include "ttAvcodec.h"
#include "ttH264.h"
#include "ttH264data.h" //FIXME FIXME FIXME (just for zigzag_scan)
#include "ttGolomb.h"

#define MAX_LOG2_MAX_FRAME_NUM    (12 + 4)
#define MIN_LOG2_MAX_FRAME_NUM    4

static const TTRational pixel_aspect[17] = {
    {   0,  1 },
    {   1,  1 },
    {  12, 11 },
    {  10, 11 },
...
const uint8_t tt_h264_chroma_qp[7][QP_MAX_NUM + 1] = {
...
static inline int decode_hrd_parameters(H264Context *h, SPS *sps)
{
    ...
    if (cpb_count > 32U) {
        ttv_log(h->avctx, TTV_LOG_ERROR, "cpb_count %d invalid\n", cpb_count);
...
static inline int decode_vui_parameters(H264Context *h, SPS *sps)
{
    ...
    if (aspect_ratio_info_present_flag) {
        ...
        } else if (aspect_ratio_idc < TT_ARRAY_ELEMS(pixel_aspect)) {
            sps->sar = pixel_aspect[aspect_ratio_idc];
        } else {
            ttv_log(h->avctx, TTV_LOG_ERROR, "illegal aspect ratio\n");
            return TTERROR_INVALIDDATA;
        }
    } else {
        sps->sar.num =
    ...
    if (show_bits1(&h->gb) && get_bits_left(&h->gb) < 10) {
        ttv_log(h->avctx, TTV_LOG_WARNING, "Truncated VUI\n");
        return 0;
    }
...
{
    int i, last = 8, next = 8;
    const uint8_t *scan = size == 16 ? zigzag_scan : tt_zigzag_direct;
...
int tt_h264_decode_seq_parameter_set(H264Context *h)
{
    ...
    if (sps_id >= MAX_SPS_COUNT) {
        ttv_log(h->avctx, TTV_LOG_ERROR, "sps_id %u out of range\n", sps_id);
        return TTERROR_INVALIDDATA;
    }
    sps = ttv_mallocz(sizeof(SPS));
    if (!sps)
...
//             ttpriv_request_sample(h->avctx, "chroma_format_idc %u",
//                                   sps->chroma_format_idc);
            goto fail;
        } else if (sps->chroma_format_idc == 3) {
            sps->residual_color_transform_flag = get_bits1(&h->gb);
            if (sps->residual_color_transform_flag) {
                ttv_log(h->avctx, TTV_LOG_ERROR, "separate color planes are not supported\n");
                goto fail;
            }
        }
...
//                                   "Different chroma and luma bit depth");
        ...
        if (sps->bit_depth_luma > 14U || sps->bit_depth_chroma > 14U) {
            ttv_log(h->avctx, TTV_LOG_ERROR, "illegal bit depth value (%d, %d)\n",
    ...
    if (sps->poc_type == 0) { // FIXME #define
        ...
        if (t>12) {
            ttv_log(h->avctx, TTV_LOG_ERROR, "log2_max_poc_lsb (%d) is out of range\n", t);
            goto fail;
        }
        ...
        if ((unsigned)sps->poc_cycle_length >=
            TT_ARRAY_ELEMS(sps->offset_for_ref_frame)) {
            ttv_log(h->avctx, TTV_LOG_ERROR,
        ...
        ttv_log(h->avctx, TTV_LOG_ERROR, "illegal POC type %d\n", sps->poc_type);
        goto fail;
    }
    ...
    if ((unsigned)sps->mb_width  >= INT_MAX / 16 ||
        ...
        ttv_image_check_size(16 * sps->mb_width,
                            16 * sps->mb_height, 0, h->avctx)) {
        ttv_log(h->avctx, TTV_LOG_ERROR, "mb_width/height overflow\n");
        goto fail;
    }
    ...
    if (sps->crop) {
        ...
        if (h->avctx->flags2 & CODEC_FLAG2_IGNORE_CROP) {
            ttv_log(h->avctx, TTV_LOG_DEBUG, "discarding sps cropping, original "
            ...
            if (crop_left  > (unsigned)INT_MAX / 4 / step_x ||
                ...
                ttv_log(h->avctx, TTV_LOG_ERROR, "crop values invalid %d %d %d %d / %d %d\n", crop_left, crop_right, crop_top, crop_bottom, width, height);
                goto fail;
            }
...
static void build_qp_table(PPS *pps, int t, int index, const int depth)
{
    ...
    for (i = 0; i < max_qp + 1; i++)
        pps->chroma_qp_table[t][i] =
            tt_h264_chroma_qp[depth - 8][ttv_clip(i + index, 0, max_qp)];
}
...
static int more_rbsp_data_in_pps(H264Context *h, PPS *pps)
{
    ...
    if ((profile_idc == 66 || profile_idc == 77 ||
        ...
        ttv_log(h->avctx, TTV_LOG_VERBOSE,
...
int tt_h264_decode_picture_parameter_set(H264Context *h, int bit_length)
{
    ...
    if (pps->slice_group_count > 1) {
        ...
        ttv_log(h->avctx, TTV_LOG_ERROR, "FMO not supported\n");
    ...
    if (pps->ref_count[0] - 1 > 32 - 1 || pps->ref_count[1] - 1 > 32 - 1) {
        ttv_log(h->avctx, TTV_LOG_ERROR, "reference overflow (pps)\n");
        goto fail;
    }
