...
//             ttv_log(h->avctx, TTV_LOG_DEBUG, "ct_type:%X pic_struct:%d\n",
...
//     if (h->avctx->debug & TT_DEBUG_BUGS)
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "user data:\"%s\"\n", user_data + 16);
...
//     if (h->avctx->debug & TT_DEBUG_PICT_INFO)
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "sei_recovery_frame_cnt: %d\n", h->sei_recovery_frame_cnt);
...
//     if (h->avctx->debug & TT_DEBUG_PICT_INFO)
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "SEI FPA %d %d %d %d %d %d\n",
...
int tt_h264_decode_sei(H264Context *h)
{
    while (get_bits_left(&h->gb) > 16 && show_bits(&h->gb, 16)) {
        ...
        if (h->avctx->debug&TT_DEBUG_STARTCODE)
            ttv_log(h->avctx, TTV_LOG_DEBUG, "SEI %d len:%d\n", type, size);

        if (size > get_bits_left(&h->gb) / 8) {
            ttv_log(h->avctx, TTV_LOG_ERROR, "SEI type %d size %d truncated at %d\n",
        ...
        case SEI_TYPE_DISPLAY_ORIENTATION:
            ...
            break;
        default:
            ttv_log(h->avctx, TTV_LOG_DEBUG, "unknown SEI type %d\n", type);
        }
