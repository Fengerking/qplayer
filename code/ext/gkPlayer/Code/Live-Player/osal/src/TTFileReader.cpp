/**
* File : TTFileReader.cpp
* Description : CTTFileReader 实现文件
*/
...
TTInt CTTFileReader::Open(const TTChar* aUrl)
{
	...
	return ((iFile != NULL) && ((TTKErrNone == fseek(iFile, 0, SEEK_END)) && ((iFileSize = TTInt64(ftell(iFile))) != TTKErrNotFound))) ? TTKErrNone : TTKErrAccessDenied;
}
...
TTInt CTTFileReader::ReadSync(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
...
TTInt CTTFileReader::ReadWait(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
	return ReadSync(aReadBuffer, aReadPos, aReadSize);
}


TTInt CTTFileReader::Read(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
	TTInt nErr = TTKErrNone;
...
TTInt CTTFileReader::CheckPreRead(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
...
void CTTFileReader::PreRead(TTInt64 aReadPos)
{
	GKASSERT(iPreReadBuffer != NULL);
...
TTInt64 CTTFileReader::Size() const
{
	return iFileSize;
}

TTUint64 CTTFileReader::BufferedSize()
{
	return iFileSize;
}
...
//	//{
//	//	GKASSERT((aReadSize > 0) && (aReadPos >= 0) && (aReadBuffer != NULL) && (iReaderObserver != NULL));
//
...
TTInt CTTFileReader::PrepareCache(TTInt64 aReadPos, TTInt aReadSize, TTInt aFlag)
{
	if(aReadPos > iFileSize)
		return TTKErrOverflow;
	
	return TTKErrNone;
}

TTInt CTTFileReader::CheckReadInt(TTInt64 aReadPos, TTInt aIntSize, TTInt& aIntOffset)
{
	if (iCurPreReadBufferPos != KReadPosInvalid)
	{
...
TTUint16 CTTFileReader::ReadUint16(TTInt64 aReadPos)// 按高字节在后面读
{
	TTInt nIntOffset = 0;
...
TTUint16 CTTFileReader::ReadUint16BE(TTInt64 aReadPos)// 按高字节在前面读
{
	TTInt nIntOffset = 0;
	if (TTKErrNone == CheckReadInt(aReadPos, sizeof(TTUint16), nIntOffset))
	{
...
TTUint32 CTTFileReader::ReadUint32(TTInt64 aReadPos)
{
	TTInt nIntOffset = 0;
...
TTUint32 CTTFileReader::ReadUint32BE(TTInt64 aReadPos)
{
	TTInt nIntOffset = 0;
	if (TTKErrNone == CheckReadInt(aReadPos, sizeof(TTUint32), nIntOffset))
	{
...
TTUint64 CTTFileReader::ReadUint64(TTInt64 aReadPos)
{
	TTInt nIntOffset = 0;
...
TTUint64 CTTFileReader::ReadUint64BE(TTInt64 aReadPos)
{
	TTInt nIntOffset = 0;
	if (TTKErrNone == CheckReadInt(aReadPos, sizeof(TTUint64), nIntOffset))
	{
