...
#ifndef __TT_FLV_TAG_H__
#define __TT_FLV_TAG_H__
...
class CTTFlvTagStream {
public:
    CTTFlvTagStream(unsigned int streamType);
	virtual ~CTTFlvTagStream();

    unsigned int type() const { return mStreamType; }

	int addTag(unsigned char *data, unsigned int size, TTInt64 timeUs);

	int	getBitrate();

	static int tagHeader(unsigned char *data, unsigned int size, int &streamType, int &dataSize, int &timeUs);
...
private:
	...
	void onPayloadData(unsigned char *data, unsigned int size, TTInt64 timeUs);
