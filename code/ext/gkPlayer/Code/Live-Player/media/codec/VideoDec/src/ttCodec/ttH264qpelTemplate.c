...
#include "ttHpelTemplate.c"
#include "ttPelTemplate.c"
...
static inline void FUNC(copy_block8)(uint8_t *dst, const uint8_t *src, int dstStride, int srcStride, int h)
{
    ...
    {
        TTV_WN4P(dst                , TTV_RN4P(src                ));
        TTV_WN4P(dst+4*sizeof(pixel), TTV_RN4P(src+4*sizeof(pixel)));
        dst+=dstStride;
        src+=srcStride;
    }
}
...
static inline void FUNC(copy_block16)(uint8_t *dst, const uint8_t *src, int dstStride, int srcStride, int h)
{
    ...
    for(i=0; i<h; i++)
    {
        TTV_WN4P(dst                 , TTV_RN4P(src                 ));
        TTV_WN4P(dst+ 4*sizeof(pixel), TTV_RN4P(src+ 4*sizeof(pixel)));
        TTV_WN4P(dst+ 8*sizeof(pixel), TTV_RN4P(src+ 8*sizeof(pixel)));
        TTV_WN4P(dst+12*sizeof(pixel), TTV_RN4P(src+12*sizeof(pixel)));
        dst+=dstStride;
        src+=srcStride;
    }
}
...
static ttv_unused void FUNC(OPNAME ## h264_qpel2_h_lowpass)(uint8_t *p_dst, const uint8_t *p_src, int dstStride, int srcStride){\
    const int h=2;\
    INIT_CLIP\
    int i;\
...
\
static ttv_unused void FUNC(OPNAME ## h264_qpel2_v_lowpass)(uint8_t *_dst, const uint8_t *_src, int dstStride, int srcStride){\
    const int w=2;\
    INIT_CLIP\
    int i;\
...
\
static ttv_unused void FUNC(OPNAME ## h264_qpel2_hv_lowpass)(uint8_t *_dst, pixeltmp *tmp, const uint8_t *_src, int dstStride, int tmpStride, int srcStride){\
    const int h=2;\
    const int w=2;\
