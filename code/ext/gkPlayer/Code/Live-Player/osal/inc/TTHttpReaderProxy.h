...
*/
#ifndef __TT_HTTP_READER_PROXY_READER_H__
#define __TT_HTTP_READER_PROXY_READER_H__
...
class CTTHttpReaderProxy : public IGKInterface
{	
public:
	/**
	* \fn							CTTHttpReaderProxy(ITTSinkDataProvider* aProvider, ITTPlayRangeObserver& aObserver);
	* \brief						构造函数	
	*/
	CTTHttpReaderProxy();

	/**
	* \fn							~CTTHttpReaderProxy();
	* \brief						析构函数	
	*/
	virtual ~CTTHttpReaderProxy();
...
private:
	...
	CTTHttpCacheFile*			iHttpCacheFile;		/**< 线程间共享，需要加锁 */
    ...
    TTChar						iCacheFile[FILE_PATH_MAX_LENGTH];
	TTChar*						iCacheUrl;	
	TTChar*						iLastUrl;
	TTInt						iLastSize;
