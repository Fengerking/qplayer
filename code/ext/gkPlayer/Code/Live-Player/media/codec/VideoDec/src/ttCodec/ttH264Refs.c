...
int tt_h264_fill_default_ref_list(H264Context *h)
{
    ...
    if (h->slice_type_nos == TTV_PICTURE_TYPE_B) {
        ...
        for (list = 0; list < 2; list++) {
            ...
            len  = build_def_list(h->default_ref_list[list], TT_ARRAY_ELEMS(h->default_ref_list[0]),
                                  sorted, len, 0, h->picture_structure);
            len += build_def_list(h->default_ref_list[list] + len,
                                  TT_ARRAY_ELEMS(h->default_ref_list[0]) - len,
        ...
        len  = build_def_list(h->default_ref_list[0], TT_ARRAY_ELEMS(h->default_ref_list[0]),
        ...
        if (get_bits1(&h->gb)) {    // ref_pic_list_modification_flag_l[01]
            ...
            for (index = 0; ; index++) {
                ...
                if (index >= h->ref_count[list]) {
                    ttv_log(h->avctx, TTV_LOG_ERROR, "reference count overflow\n");
                    return -1;
                }
                ...
                case 0:
                case 1: {
                    const unsigned int abs_ditt_pic_num = get_ue_golomb(&h->gb) + 1;
                    int frame_num;

                    if (abs_ditt_pic_num > h->max_pic_num) {
                        ttv_log(h->avctx, TTV_LOG_ERROR,
                               "abs_ditt_pic_num overflow\n");
                        return TTERROR_INVALIDDATA;
                    }

                    if (modification_of_pic_nums_idc == 0)
                        pred -= abs_ditt_pic_num;
                    else
                        pred += abs_ditt_pic_num;
    ...
    for (list = 0; list < h->list_count; list++) {
        for (index = 0; index < h->ref_count[list]; index++) {
            if (   !h->ref_list[list][index].f.buf[0]
                ...
                int i;
                ttv_log(h->avctx, TTV_LOG_ERROR, "Missing reference picture, default is %d\n", h->default_ref_list[list][0].poc);
            ...
            ttv_assert0(ttv_buffer_get_ref_count(h->ref_list[list][index].f.buf[0]) > 0);
        }
    }
...
//         if (h->avctx->debug & TT_DEBUG_MMCO)
//             ttv_log(h->avctx, TTV_LOG_DEBUG, "%d %d %p\n", i, pic->frame_num, pic);
...
//     if (h->avctx->debug & TT_DEBUG_MMCO)
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "remove short %d count %d\n", frame_num, h->short_ref_count);
...
//    uint32_t i;
//     if (h->avctx->debug & TT_DEBUG_MMCO) {
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "short term list:\n");
//         for (i = 0; i < h->short_ref_count; i++) {
//             H264Picture *pic = h->short_ref[i];
//             ttv_log(h->avctx, TTV_LOG_DEBUG, "%"PRIu32" fn:%d poc:%d %p\n",
...
//     uint32_t i;
//     if (h->avctx->debug & TT_DEBUG_MMCO) {
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "long term list:\n");
//         for (i = 0; i < 16; i++) {
...
static int check_opcodes(MMCO *mmco1, MMCO *mmco2, int n_mmcos)
{
    ...
    for (i = 0; i < n_mmcos; i++) {
        if (mmco1[i].opcode != mmco2[i].opcode) {
            ttv_log(NULL, TTV_LOG_ERROR, "MMCO opcode [%d, %d] at %d mismatches between slices\n",
...
int tt_h264_execute_ref_pic_marking(H264Context *h, MMCO *mmco, int mmco_count)
{
    ...
    H264Picture *ttv_uninit(pic);

//     if ((h->avctx->debug & TT_DEBUG_MMCO) && mmco_count == 0)
//         ttv_log(h->avctx, TTV_LOG_DEBUG, "no mmco here\n");

    for (i = 0; i < mmco_count; i++) {
        int ttv_uninit(structure), ttv_uninit(frame_num);
//         if (h->avctx->debug & TT_DEBUG_MMCO)
//             ttv_log(h->avctx, TTV_LOG_DEBUG, "mmco:%d %d %d\n", h->mmco[i].opcode,
        ...
        if (mmco[i].opcode == MMCO_SHORT2UNUSED ||
            ...
            if (!pic) {
                if (mmco[i].opcode != MMCO_SHORT2LONG ||
                    ...
                    ttv_log(h->avctx, h->short_ref_count ? TTV_LOG_ERROR : TTV_LOG_DEBUG, "mmco: unref short failure\n");
                    err = TTERROR_INVALIDDATA;
                }
                continue;
            }
        }
...
//            if (h->avctx->debug & TT_DEBUG_MMCO)
//                 ttv_log(h->avctx, TTV_LOG_DEBUG, "mmco: unref short %d count %d\n",
//                        h->mmco[i].short_pic_num, h->short_ref_count);
        ...
        case MMCO_LONG2UNUSED:
			...
			else if (h->avctx->debug & TT_DEBUG_MMCO)
                ttv_log(h->avctx, TTV_LOG_DEBUG, "mmco: unref long failure\n");
            break;
        case MMCO_LONG:
            ...
            if (h->short_ref[0] == h->cur_pic_ptr) {
                ttv_log(h->avctx, TTV_LOG_ERROR, "mmco: cannot assign current picture to short and long at the same time\n");
                ...
                if (h->cur_pic_ptr->long_ref) {
                    for(j=0; j<16; j++) {
                        if(h->long_ref[j] == h->cur_pic_ptr) {
                            remove_long(h, j, 0);
                            ttv_log(h->avctx, TTV_LOG_ERROR, "mmco: cannot assign current picture to 2 long term references\n");
                        }
                    }
                }
                ttv_assert0(!h->cur_pic_ptr->long_ref);
                remove_long(h, mmco[i].long_arg, 0);
    ...
    if (!current_ref_assigned) {
        ...
        if (h->short_ref_count && h->short_ref[0] == h->cur_pic_ptr) {
            ...
            ttv_log(h->avctx, TTV_LOG_ERROR, "illegal short term reference "
            ...
            if (pic) {
                ttv_log(h->avctx, TTV_LOG_ERROR, "illegal short term buffer state detected\n");
                err = TTERROR_INVALIDDATA;
            }
    ...
    pps_count = 0;
    for (i = 0; i < TT_ARRAY_ELEMS(h->pps_buffers); i++)
    ...
    if (   err >= 0
        ...
        && h->cur_pic_ptr->f.pict_type == TTV_PICTURE_TYPE_I){
        h->cur_pic_ptr->recovered |= 1;
        if(!h->avctx->has_b_frames)
            h->frame_recovered |= FRAME_RECOVERED_SEI;
    }

    return (h->avctx->err_recognition & TTV_EF_EXPLODE) ? err : 0;
}
                                   ...
                                   int first_slice)
{
    ...
    if (h->nal_unit_type == NAL_IDR_SLICE) { // FIXME fields
        ...
        if (get_bits1(gb)) { // adaptive_ref_pic_marking_mode_flag
            ...
            if (first_slice) {
                ret = tt_generate_sliding_window_mmcos(h, first_slice);
