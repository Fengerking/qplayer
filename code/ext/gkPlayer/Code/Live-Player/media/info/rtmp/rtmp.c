...
int g_SokectErrorCode = 0;
int g_ExptionType = 0;

int GetErrorCode()
{
	return g_SokectErrorCode;
}
...
{
    LOGI("RTMP PACKET: packet type: 0x%02x. channel: 0x%02x. info 1: %d info 2: %d. Body size: %u. body: 0x%02x",
...
RTMP_Init(RTMP *r)
{
    ...
    r->Link.timeout = 5;//default 30
    r->Link.swfAge = 30;
	r->m_ip = 0;
	r->m_cancle = 0;
    
    //cs
    memset(&r->m_vecChannelsIn, 0, sizeof(hash_head_t));
    memset(&r->m_vecChannelsOut, 0, sizeof(hash_head_t));
    setHashValueType(&r->m_channelTimestamp, HASH_VALUETYPE_INT);
    
    r->Link.extras.o_num = 0;
    r->Link.extras.o_props = NULL;
    r->Link.swfUrl.av_len = 0;
    r->Link.swfUrl.av_val = NULL;
    r->Link.pageUrl.av_len = 0;
    r->Link.pageUrl.av_val = NULL;
    r->Link.auth.av_len = 0;
    r->Link.auth.av_val = NULL;
    
    r->Link.hostname.av_len = 0;
    r->Link.hostname.av_val = NULL;
    
    r->Link.sockshost.av_len = 0;
    r->Link.sockshost.av_val = NULL;
    
    r->Link.playpath0.av_len = 0;
    r->Link.playpath0.av_val = NULL;
    
    r->Link.playpath.av_len = 0;
    ...
    r->Link.flashVer.av_len = 0;
    r->Link.flashVer.av_val = NULL;
    
    r->Link.subscribepath.av_len = 0;;
    r->Link.subscribepath.av_val = NULL;
    
    r->Link.token.av_len = 0;;
    r->Link.token.av_val = NULL;
    
    r->m_nServerStreamEof = 0;
}
...
{
    LOGI("Protocol : %s", RTMPProtocolStrings[protocol&7]);
    LOGI("Hostname : %.*s", host->av_len, host->av_val);
    LOGI("Port     : %d", port);
    LOGI("Playpath : %s", playpath->av_val);
    
    if (tcUrl && tcUrl->av_val)
        LOGI("tcUrl    : %s", tcUrl->av_val);
    if (swfUrl && swfUrl->av_val)
        LOGI("pageUrl  : %s", pageUrl->av_val);
    if (app && app->av_val)
        LOGI("app      : %.*s", app->av_len, app->av_val);
    //if (auth && auth->av_val)
    //    LOGI("auth     : %s", auth->av_val);
    //if (subscribepath && subscribepath->av_val)
    //    LOGI("subscribepath : %s", subscribepath->av_val);
    //if (flashVer && flashVer->av_val)
    //    LOGI("flashVer : %s", flashVer->av_val);
    //if (dStart > 0)
    //    LOGI("StartTime     : %d msec", dStart);
    //if (dStop > 0)
    //    LOGI("StopTime      : %d msec", dStop);
    
    LOGI("timeout  : %d sec", timeout);
    ...
    if (swfSHA256Hash != NULL && swfSize > 0)
    {
        ...
        LOGI("SWFSize  : %lu", r->Link.SWFSize);
    }
    else
    {
        r->Link.SWFSize = 0;
    ...
    if (sockshost->av_len)
    {
        ...
        LOGI("Connecting via SOCKS proxy: %s:%d", r->Link.sockshost.av_val,
                 r->Link.socksport);
    }
    else
    {
...
int RTMP_SetOpt(RTMP *r, const AVal *opt, AVal *arg)
{
    ...
    if (!options[i].name.av_len) {
        LOGE("Unknown option %s", opt->av_val);
        return FALSE;
    }
...
static int
add_addr_info(struct sockaddr *service, AVal *host, int port)
{
    char *hostname;
    int ret = TRUE;
    ...
    {
        LOGE("Problem accessing the DNS. (addr: %s)", hostname);
        ret = FALSE;
        goto finish;
    }
    ...
    if (addr != NULL)
    {
        memcpy( service, addr->ai_addr, addr->ai_addrlen);
...
int waitsocketwritebuffer(int aSocketHandle, struct timeval aTimeOut)
{
	fd_set fds;
	int ret;
	int err = 0;
	...
	ret = select(aSocketHandle + 1, NULL, &fds, NULL, &aTimeOut);
...
RTMP_Connect0(RTMP *r, struct sockaddr * service)
{
    ...
    {
        setsocketnonblock(r->m_sb.sb_socket);
     
#ifdef __TT_OS_IOS__
        //set SO_NOSIGPIPE for sockect (IOS)
        setsockopt(r->m_sb.sb_socket, SOL_SOCKET, SO_NOSIGPIPE, (void *)&on, sizeof(on));
#endif

		nErr = connect(r->m_sb.sb_socket, service, sizeof(struct sockaddr));
        if (nErr < 0)
		{
#ifdef __TT_OS_WINDOWS__
			if(nErr == -1)
...
#endif
			{
				nErr =  waitsocketwritebuffer(r->m_sb.sb_socket,timeout);
			}

            setsocketblock(r->m_sb.sb_socket);

			if (nErr < 0)
			{
				g_SokectErrorCode = GetSockError();
				LOGE("failed to connect socket. %d (%s)", g_SokectErrorCode, strerror(g_SokectErrorCode));
				RTMP_Close(r);
        ...
        if (r->Link.socksport)
        {
           LOGI("SOCKS negotiation");
            if (!SocksNegotiate(r))
            {
                LOGE("SOCKS negotiation failed.");
                RTMP_Close(r);
    ...
    {
        LOGE("failed to create socket. Error: %d",GetSockError());
        return TTKErrBadHandle;
    }
    ...
    /* set timeout */
    {
        ...
        {
            LOGE("Setting socket timeout to %ds failed!", r->Link.timeout);
        }
    }
...
RTMP_Connect1(RTMP *r, RTMPPacket *cp)
{
    if (r->Link.protocol & RTMP_FEATURE_SSL)
    {
        ...
        {
            LOGE("connect 2st, TLS_Connect failed");
            RTMP_Close(r);
            return TTKErrCouldNotConnect;
        }
#else
        LOGE("connect stream, no SSL/TLS support");
        RTMP_Close(r);
        return TTKErrCouldNotConnect;
    ...
    LOGI("connect stream, handshaking start");
    if (!HandShake(r, TRUE))
    {
        LOGE("connect stream, handshake failed.");
        RTMP_Close(r);
        return TTKErrHandshakeFailed;
    }
    LOGI("connect stream, handshaked");
    
    if (!SendConnectPacket(r, cp))
    {
        LOGE("connect stream, Send ConnectPacket failed.");
        RTMP_Close(r);
        return TTKErrSendConnPacketFailed;
    }
...
RTMP_Connect(RTMP *r, RTMPPacket *cp)
{
    ...
    struct sockaddr* service = (struct sockaddr*)(&store);
    if (!r->Link.hostname.av_len)
        return FALSE;
    
    //emset(&service, 0, sizeof(struct sockaddr));
    memset(&store, 0, sizeof(struct sockaddr_storage));
    
    if (r->Link.socksport)
    {
        /* Connect via SOCKS */
        if (!add_addr_info(service, &r->Link.sockshost, r->Link.socksport))
            return TTKErrDNSFailed;
    }
    else
    {
        /* Connect directly */
    ...
    if (service->sa_family == AF_INET) {
        struct	in_addr aIP = ((struct sockaddr_in*)service)->sin_addr;
        char* pHostIP = inet_ntoa(aIP);
        r->m_ip = inet_addr(pHostIP);
    } else  if (service->sa_family == AF_INET6){
        r->m_ip = 0xffffffff;
    }
    
	//r->m_ip = service.sin_addr.s_addr; //TODO
...
SocksNegotiate(RTMP *r)
{
    ...
    
    {
        ...
        {
            LOGE("SOCKS returned error code %d", packet[1]);
            return FALSE;
        }
    }
}
...
RTMP_ConnectStream(RTMP *r, int seekTime)
{
    ...
    while (!r->m_bPlaying && RTMP_IsConnected(r) && RTMP_ReadPacket(r, &packet))
    {
        if (RTMPPacket_IsReady(&packet))
        {
            ...
            {
                LOGI("Received FLV packet before play()! Ignoring.");
...
RTMP_GetNextMediaPacket(RTMP *r, RTMPPacket *packet)
{
           ...
           && RTMP_ReadPacket(r, packet))
    {
        ...
        else if (r->m_pausing == 3)
        {
            if (packet->m_nTimeStamp <= r->m_mediaStamp)
            {
                ...
                /*LOGE("Skipped type: %02X, size: %d, TS: %d ms, abs TS: %d, pause: %d ms",
    ...
    if (bHasMediaPacket)
        ...
        r->m_pauseStamp = (uint32_t)getValueByKey(&r->m_channelTimestamp, r->m_mediaChannel);//r->m_channelTimestamp[r->m_mediaChannel];
...
RTMP_ClientPacket(RTMP *r, RTMPPacket *packet)
{
    ...
    switch (packet->m_packetType)
    {
        ...
        case 0x03:
            /* bytes read report */
			//cs
            //LOGI("Parse server packet, received: bytes read report");
            break;
        ...
        case 0x08:
            /* audio data */
            /*LOGI( "%s, received: audio %lu bytes", __FUNCTION__, packet.m_nBodySize); */
        ...
        case 0x09:
            /* video data */
            /*LOGI( "%s, received: video %lu bytes", __FUNCTION__, packet.m_nBodySize); */
        ...
        case 0x0F:			/* flex stream send */
            LOGI("Parse server packet, flex stream send, size %d bytes, not supported, ignoring",packet->m_nBodySize);
            break;
            
        case 0x10:			/* flex shared object */
            LOGI("Parse server packet, flex shared object, size %d bytes, not supported, ignoring", packet->m_nBodySize);
            break;
            
        case 0x11:			/* flex message */
        {
            LOGI("Parse server packet, flex message, size %u bytes, not fully supported", packet->m_nBodySize);
            ...
            if(nRes < 0) {
                LOGE("%s, error decoding AMF3 packet");
                /*return; */
            }
        ...
        case 0x12:
            /* metadata (notify) */
            LOGI("Parse server packet, received: notify %u bytes",packet->m_nBodySize);
            if (HandleMetadata(r, packet->m_body, packet->m_nBodySize))
                bHasMediaPacket = 1;
            break;
            
        case 0x13:
            LOGI("Parse server packet, shared object, not supported, ignoring");
            break;
            
        case 0x14:
            /* invoke */
            LOGI("Parse server packet, received: invoke %u bytes",packet->m_nBodySize);
            /*RTMP_LogHex(packet.m_body, packet.m_nBodySize); */
        ...
        case 0x16:
        {
            ...
            while (pos + 11 < packet->m_nBodySize)
            {
                ...
                {
                    LOGI("Parse server packet,Stream corrupt?!");
                    break;
                }
            ...
            /* FLV tag(s) */
            /*LOGI( "%s, received: FLV tag(s) %lu bytes", __FUNCTION__, packet.m_nBodySize); */
            bHasMediaPacket = 1;
            break;
        }
        default:
            LOGI("Parse server packet, unknown packet type received: 0x%02x",packet->m_packetType);

            //LOGE("", packet->m_body, packet->m_nBodySize);
    }
...
ReadN(RTMP *r, char *buffer, int n)
{
    ...
    while (n > 0)
    {
        ...
        if (nBytes == 0)
        {
            LOGI("RTMP socket closed by peer");
            /*goto again; */
            RTMP_Close(r);
            break;
        }
...
WriteN(RTMP *r, const char *buffer, int n)
{
    ...
    while (n > 0)
    {
        ...
        {
			g_SokectErrorCode = GetSockError();
			LOGE("RTMP send error %d (%d bytes) %s", g_SokectErrorCode, n,strerror(g_SokectErrorCode));

			if (g_SokectErrorCode == EINTR && r->m_cancle == 0)
				continue;

			//if (g_SokectErrorCode == EINTR && !RTMP_ctrlC)
			//	continue;

#ifndef __TT_OS_WINDOWS__
			LOGE("start to close rtmp link!");
...
SendFCSubscribe(RTMP *r, AVal *subscribepath)
{
    ...
    LOGI("FCSubscribe: %s", subscribepath->av_val);
    enc = packet.m_body;
...
RTMP_SendPause(RTMP *r, int DoPause, int iTime)
{
    ...
    *enc++ = AMF_NULL;
    ...
    LOGI("SendPause, %d, pauseTime=%d", DoPause, iTime);
    return RTMP_SendPacket(r, &packet, TRUE);
}
...
SendBytesReceived(RTMP *r)
{
    ...
    /*LOGI( "Send bytes report. 0x%x (%d bytes)", (unsigned int)m_nBytesIn, m_nBytesIn); */
    //cs
    LOGI("sent BytesReceived");
    return RTMP_SendPacket(r, &packet, FALSE);
}
...
SendPlay(RTMP *r)
{
    ...
    *enc++ = AMF_NULL;
    ...
    LOGI("SendPlay seekTime=%d, stopTime=%d, sending play: %s", r->Link.seekTime, r->Link.stopTime,
...
RTMP_SendCtrl(RTMP *r, short nType, unsigned int nObject, unsigned int nTime)
{
    ...
    LOGI("sending ctrl. type: 0x%04x", (unsigned short)nType);
    ...
    if (nType == 0x1B)
    {
        ...
        LOGI("Sending SWFVerification response: ");
...
HandleInvoke(RTMP *r, const char *body, unsigned int nBodySize)
{
    ...
    {
        LOGI("RPC, Sanity failed. no string method in invoke packet");
        return 0;
    }
    ...
    if (nRes < 0)
    {
        LOGE("RPC, error decoding invoke packet");
        return 0;
    }
    //cs
    //AMF_Dump(&obj);
    ...
    LOGI("RPC, server invoking <%s>", method.av_val);
    ...
    if (AVMATCH(&method, &av__result))
    {
        ...
        if (!methodInvoked.av_val) {
            LOGI("RPC, received result id %d without matching request",txn);
            goto leave;
        }
        
        LOGI("RPC,, received result for method call <%s>",methodInvoked.av_val);
        
        if (AVMATCH(&methodInvoked, &av_connect))
        {
            LOGI("server: connect ok");
    ...
    else if (AVMATCH(&method, &av_onStatus))
    {
        ...
        LOGI("server: onStatus: %s",code.av_val);
        ...
        {
            //cs
            LOGE("server: NetStream fail.playfail.notfound.invalidapp %s", code.av_val);
            r->m_stream_id = -1;
            RTMP_Close(r);
        }
        ...
        {
            int i;
            LOGI("server: NetStream.Play.Start");
        ...
        {
            LOGI("server: NetStream.Play.Complete.stop");
            RTMP_Close(r);
            ret = 1;
        }
        ...
        {
            LOGI("server: NetStream.Pause.Notify");
        ...
        if (prop->p_type != AMF_OBJECT)
        {
            ...
            if (prop->p_name.av_len)
            {
                ...
                if (strlen(str) >= 1 && str[strlen(str) - 1] == '\n')
                    str[strlen(str) - 1] = '\0';
                LOGI("  %-22.*s%s", prop->p_name.av_len, prop->p_name.av_val, str);
            }
        }
        else
        {
            if (prop->p_name.av_len)
                LOGI( "%.*s:", prop->p_name.av_len, prop->p_name.av_val);
...
HandleMetadata(RTMP *r, char *body, unsigned int len)
{
    ...
    if (nRes < 0)
    {
        LOGE("Metadata, error decoding meta data packet");
        return FALSE;
    }
    //cs
    //AMF_Dump(&obj);
    ...
    if (AVMATCH(&metastring, &av_onMetaData))
    {
        ...
        if (RTMP_FindFirstMatchingProperty(&obj, &av_duration, &prop))
        {
            ...
            /*LOGI( "Set duration: %.2f", m_fDuration); */
        }
...
HandleChangeChunkSize(RTMP *r, const RTMPPacket *packet)
{
    if (packet->m_nBodySize >= 4)
    {
        ...
        LOGI("received: chunk size change to %d",r->m_inChunkSize);
    }
}
...
HandleCtrl(RTMP *r, const RTMPPacket *packet)
{
    ...
    LOGI(" received ctrl. type: %d, len: %d", nType,packet->m_nBodySize);
    /*RTMP_LogHex(packet.m_body, packet.m_nBodySize); */
    ...
    if (packet->m_nBodySize >= 6)
    {
        switch (nType)
        {
            case 0:
                ...
                LOGI("ServerCtrl, Stream Begin %d", tmp);
                break;
                
            case 1:
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                r->m_nServerStreamEof = 1;
                LOGI("ServerCtrl, Stream EOF %d", tmp);
                if (r->m_pausing == 1)
                    r->m_pausing = 2;
                break;
            ...
            case 2:
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                LOGI("ServerCtrl, Stream Dry %d", tmp);
                break;
                
            case 4:
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                LOGI("ServerCtrl, Stream IsRecorded %d", tmp);
                break;
                
            case 6:		/* server ping. reply with pong. */
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                LOGI("ServerCtrl, Ping %d", tmp);
            ...
            case 31:
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                LOGI("ServerCtrl, Stream BufferEmpty %d", tmp);
            ...
            case 32:
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                LOGI("ServerCtrl, Stream BufferReady %d", tmp);
                break;
                
            default:
                tmp = AMF_DecodeInt32(packet->m_body + 2);
                LOGI("ServerCtrl,, Stream xx %d", tmp);
                break;
        }
    ...
    if (nType == 0x1A)
    {
        LOGI("ServerCtrl, SWFVerification ping received");
#ifdef CRYPTO
        /*RTMP_LogHex(packet.m_body, packet.m_nBodySize); */
        ...
        {
            LOGE("ServerCtrl,: Ignoring SWFVerification request, use --swfVfy!");
        }
#else
        LOGI("ServerCtrl: Ignoring SWFVerification request, no CRYPTO support!");
#endif
    }
}
...
HandleServerBW(RTMP *r, const RTMPPacket *packet)
{
    ...
    LOGI("ServerCtrl: server BW = %d", r->m_nServerBW);
}
...
HandleClientBW(RTMP *r, const RTMPPacket *packet)
{
    ...
    if (packet->m_nBodySize > 4)
        ...
        r->m_nClientBW2 = -1;
    LOGI("ServerCtrl: client BW = %d %d", r->m_nClientBW, r->m_nClientBW2);
}
...
RTMP_ReadPacket(RTMP *r, RTMPPacket *packet)
{
    ...
    int didAlloc = FALSE;
    RTMPPacket* packetIn = NULL;
    int aTimeStamp;
    
    if (ReadN(r, (char *)hbuf, 1) == 0)
    {
        LOGE("ReadPacket, failed to read RTMP packet header");
        return FALSE;
    }
    ...
    if (packet->m_nChannel == 0)
    {
        ...
        {
            LOGE("ReadPacket, failed to read RTMP packet header 2nd byte");
            return FALSE;
        }
    ...
    else if (packet->m_nChannel == 1)
    {
        ...
        {
            LOGE("ReadPacket, failed to read RTMP packet header 3nd byte" );
            return FALSE;
        }
        tmp = (hbuf[2] << 8) + hbuf[1];
        packet->m_nChannel = tmp + 64;
        LOGI("ReadPacket, m_nChannel: %0x", packet->m_nChannel);
        header += 2;
    }
    ...
    {				/* using values from the last message of this channel */
        packetIn = (RTMPPacket*)getValueByKey(&(r->m_vecChannelsIn), packet->m_nChannel);
        if (packetIn)
            memcpy(packet, packetIn,sizeof(RTMPPacket));
    }
    ...
    {
        LOGE("ReadPacket, failed to read RTMP packet header. type: %x", (unsigned int)hbuf[0]);
        return FALSE;
    }
    ...
    if (nSize >= 3)
    {
        ...
        /*LOGI( "%s, reading RTMP packet chunk on channel %x, headersz %i, timestamp %i, abs timestamp %i", __FUNCTION__, packet.m_nChannel, nSize, packet.m_nTimeStamp, packet.m_hasAbsTimestamp); */
        ...
        if (packet->m_nTimeStamp == 0xffffff)
        {
            ...
            {
                LOGE("ReadPacket, failed to read extended timestamp");
                return FALSE;
            }
    ...
    if (packet->m_nBodySize > 0 && packet->m_body == NULL)
    {
        ...
        {
            LOGE("ReadPacket, failed to allocate packet");
            return FALSE;
        }
        didAlloc = TRUE;
        packet->m_headerType = (hbuf[0] & 0xc0) >> 6;
    ...
    {
        LOGE("ReadPacket, failed to read RTMP packet body. len: %lu",packet->m_nBodySize);
        return FALSE;
    }
    ...
    packetIn = (RTMPPacket*)getValueByKey(&(r->m_vecChannelsIn), packet->m_nChannel);
    if (!packetIn){
        packetIn = (RTMPPacket *)malloc(sizeof(RTMPPacket));
        putValueByKey(&(r->m_vecChannelsIn), packet->m_nChannel, packetIn);
    }
    memcpy(packetIn, packet, sizeof(RTMPPacket));
    
    if (RTMPPacket_IsReady(packet))
    {
        /* make packet's timestamp absolute */
        aTimeStamp = (int)getValueByKey(&r->m_channelTimestamp, packet->m_nChannel);
        if (!packet->m_hasAbsTimestamp)
            packet->m_nTimeStamp += aTimeStamp;	/* timestamps seem to be always relative!! */
        
        if (r->m_channelTimestamp.findStatus)
            updateValueByKey(&r->m_channelTimestamp, packet->m_nChannel, packet->m_nTimeStamp);
        else
            putValueByKey(&r->m_channelTimestamp, packet->m_nChannel, packet->m_nTimeStamp);
        
        /* reset the data from the stored packet. we keep the header since we may use it later if a new packet for this channel */
        /* arrives and requests to re-use some info (small packet header) */
        packetIn->m_body = NULL;
        packetIn->m_nBytesRead = 0;
        packetIn->m_hasAbsTimestamp = FALSE;	/* can only be false if we reuse header */
    }
    else
    {
...
HandShake(RTMP *r, int FP9HandShake)
{
    ...
    LOGI("HandShake: Type Answer   : %02X", type);
    
    if (type != clientbuf[0])
        LOGI("HandShake: Type mismatch: client sent %d, server answered %d", clientbuf[0], type);
    ...
    LOGI("HandShake,Server Uptime %d:   FMS Version : %d.%d.%d.%d", suptime,
    ...
    if (!bMatch)
    {
        LOGI("HandShake, client signature does not match!");
    }
    return TRUE;
}
...
SHandShake(RTMP *r)
{
    ...
    LOGI("SHandShake: Type Request  : %02X", serverbuf[0]);
    
    if (serverbuf[0] != 3)
    {
        LOGE("SHandShake: Type unknown: client sent %02X", serverbuf[0]);
        return FALSE;
    }
    ...
    //LOGI( "%s: Client Uptime : %d", __FUNCTION__, uptime);
    LOGI("SHandShake: Player Version: %d.%d.%d.%d",clientsig[4], clientsig[5], clientsig[6], clientsig[7]);
    ...
    if (!bMatch)
    {
        LOGI("SHandShake, client signature does not match!");
    }
    return TRUE;
}
#endif
...
RTMP_SendPacket(RTMP *r, RTMPPacket *packet, int queue)
{
    ...
    prevPacket = (RTMPPacket*)getValueByKey(&(r->m_vecChannelsOut), packet->m_nChannel);
    ...
    {
        LOGE("sanity failed!! trying to send header of type: 0x%02x.",(unsigned char)packet->m_headerType);
        return FALSE;
    }
    ...
    *hptr++ = c;
    ...
    //LOGI("Sendpacket: fd=%d, size=%d", r->m_sb.sb_socket, nSize);
    ...
    if (packet->m_packetType == 0x14)
    {
        ...
        LOGI("add Invoking %s", method.av_val);
    ...
    packetOut = (RTMPPacket*)getValueByKey(&(r->m_vecChannelsOut), packet->m_nChannel);
    if (!packetOut){
        packetOut = (RTMPPacket *)malloc(sizeof(RTMPPacket));
        putValueByKey(&(r->m_vecChannelsOut), packet->m_nChannel, packetOut);
    }
    memcpy(packetOut, packet, sizeof(RTMPPacket));
    return TRUE;
}
...
RTMP_Close(RTMP *r)
{
    ...
    destroyHashTable(&r->m_vecChannelsIn);
    destroyHashTable(&r->m_vecChannelsOut);
    destroyHashTable(&r->m_channelTimestamp);
...
int waitsocketreadbuffer(RTMP *r, struct timeval* aTimeOut)
{
    fd_set fds;
    int nRet;
    int errorNum =0;
retry:
    ...
    nRet = select(r->m_sb.sb_socket + 1, &fds, NULL, NULL, aTimeOut);
    if (nRet > 0 && !FD_ISSET(r->m_sb.sb_socket, &fds))
    {
        nRet = 0;
    }
    else if(nRet < 0)
    {
        errorNum = errno;
        if ((errorNum == ETIMEDOUT || errorNum == EINTR) && !(r->m_cancle))//&& IsCancel() == ETTFalse)
        {
           // tryCnt++;
            goto retry;
        }
...
RTMPSockBuf_Fill(RTMP *r)
{
    ...
    while (1)
    {
        ...
        ret = waitsocketreadbuffer(r, &timeout);
        ...
        if (sb->sb_ssl)
        {
            ...
            else
            {
                ...
                LOGE("SockBuf.Fill, recv returned %d. GetSockError(): %d (%s)", nBytes, sockerr, strerror(sockerr));
                if (sockerr == EINTR)
                    continue;
...
#endif
        {
            if (ret > 0) {
                ...
                if (nBytes <= 0) { //0 : close by peer, -1 : network disconnet phisycally
                    g_SokectErrorCode = GetSockError();
                    LOGE("SockBuf.Fill, recv returned %d. GetSockError(): %d (%s)", nBytes, g_SokectErrorCode, strerror(g_SokectErrorCode));
...
Read_1_Packet(RTMP *r, char *buf, unsigned int buflen)
{
    ...
    while (rtnGetNextMediaPacket)
	{
        ...
        {
            LOGI("Got Play.Complete or Play.Stop from server. Assuming stream is complete");
        ...
        {
            LOGI("ignoring too small video packet: size: %d", nPacketLen);
            ret = RTMP_READ_IGNORE;
            break;
        }
        if (packet.m_packetType == 0x08 && nPacketLen <= 1)
        {
            LOGI( "ignoring too small audio packet: size: %d", nPacketLen);
            ret = RTMP_READ_IGNORE;
            break;
        }
        ...
        /*LOGE("type: %02X, size: %d, TS: %d ms, abs TS: %d",
        ...
        if (r->m_read.flags & RTMP_READ_RESUME)
        {
            ...
            if (packet.m_nTimeStamp == 0)
            {
                ...
                if (r->m_read.nInitialFrameSize > 0)
                {
                        ...
                        && r->m_read.nInitialFrameSize == nPacketLen)
                    {
                        ...
                        {
                           // LOGI("Checked keyframe successfully!");
                    ...
                    if (packet.m_packetType == 0x16)
                    {
                        ...
                        while (pos + 11 < nPacketLen)
                        {
                            ...
                            /*LOGE("keyframe search: FLV Packet: type %02X, dataSize: %d, timeStamp: %d ms",
                                ...
                                /* && (packetBody[11]&0xf0) == 0x10 */ )
                            {
                                ...
                                {
                                    LOGI("Found keyframe with resume-keyframe timestamp!");
                                    if (r->m_read.nInitialFrameSize != dataSize
                                        || memcmp(r->m_read.initialFrame,
                                                  ...
                                                  nInitialFrameSize) != 0)
                                    {
                                        LOGE("FLV Stream: Keyframe doesn't match!");
                                        ret = RTMP_READ_ERROR;
                                        break;
                                    }
                                    ...
                                    {
                                        LOGI("Non skipable packet since it doesn't end with chunk, stream corrupt!");
                                        ret = RTMP_READ_ERROR;
                                        break;
                                    }
                        ...
                        if (ts < r->m_read.nResumeTS)
                        {
                            LOGE( "First packet does not contain keyframe, all "
                    ...
                    stopKeyframeSearch:
                        ...
                        {
                            LOGE("Couldn't find the seeked keyframe in this chunk!");
                            ret = RTMP_READ_IGNORE;
                            break;
                        }
                    }
            ...
            {
                LOGI("Stream does not start with requested frame, ignoring data... ");
            ...
            if (!(r->m_read.flags & RTMP_READ_GOTFLVK) &&
                packet.m_packetType == 0x16)
            {
                LOGI("Stream does not start with requested FLV frame, ignoring data... ");
        ...
        if (size + 4 > buflen)
        {
            ...
            if (r->m_read.buf == 0)
            {
                ...
                ret = RTMP_READ_NOMEMORY;		/* fatal error */
                break;
            }
            recopy = TRUE;
            ptr = r->m_read.buf;
            ...
            || packet.m_packetType == 0x12)
        {
            ...
            *ptr = packet.m_packetType;
            ...
            if(packet.m_packetType == 0x09) { /* video */
                ...
                LOGI("VIDEO: nTimeStamp: 0x%08X (%d)\n", nTimeStamp, nTimeStamp);
            }
#endif
        ...
        if (packet.m_packetType == 0x16)
        {
            ...
            while (pos + 11 < nPacketLen)
            {
                ...
                if (pos + 11 + dataSize + 4 > nPacketLen)
                {
                    ...
                    {
                        LOGE("Wrong data size (%u), stream corrupted, aborting!",dataSize);
                        ret = RTMP_READ_NETDATAERROR;
                        break;
                    }
                    LOGI("No tagSize found, appending!");
                ...
                else
                {
                    ...
                    /*LOGE("FLV Packet: type %02X, dataSize: %lu, tagSize: %lu, timeStamp: %lu ms",
                             (unsigned char)packetBody[pos], dataSize, prevTagSize,
                             nTimeStamp);*/
                    
                    if (prevTagSize != (dataSize + 11))
                    {
                        /*LOGE("Tag and data size are not consitent, writing tag size according to dataSize+11: %d",dataSize + 11);*/
         ...
         */
        /* LOGI( "type: %02X, size: %d, pktTS: %dms, TS: %dms, bLiveStream: %d", packet.m_packetType, nPacketLen, packet.m_nTimeStamp, nTimeStamp, r->Link.lFlags & RTMP_LF_LIVE); */
    ...
    if (rtnGetNextMediaPacket){
        RTMPPacket_Free(&packet);
        //cs
		//LOGE("[dataype = %d %d]\t",packet.m_packetType,packet.m_nBodySize);
    }
    if (recopy)
    {
    ...
    if (rtnGetNextMediaPacket == 0){
        if (RTMP_IsConnected(r))
            ret = RTMP_READ_TIMEOUT;
        else
            ret = RTMP_READ_PEER_DISCONNET;
    }
...
fail:
    switch (r->m_read.status) {
        case RTMP_READ_EOF:
        case RTMP_READ_TIMEOUT:
        case RTMP_READ_COMPLETE:
            return 0;
        case RTMP_READ_ERROR:  /* corrupted stream, resume failed */
        case RTMP_READ_NOMEMORY:
		case RTMP_READ_NETDATAERROR:
		case RTMP_READ_CANCLE:
        case RTMP_READ_PEER_DISCONNET:
            SetSockError(EINVAL);
            return -1;
        default:
            break;
...
RTMP_Write(RTMP *r, const char *buf, int size)
{
    ...
    while (s2)
    {
        if (!pkt->m_nBytesRead)
        {
            ...
            {
                LOGE( "rtmp.write, failed to allocate packet");
                return FALSE;
            }
            enc = pkt->m_body;
