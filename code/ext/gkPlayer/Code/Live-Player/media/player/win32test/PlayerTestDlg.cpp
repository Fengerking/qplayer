...
#include "TTAudioEffect.h"
#include "TTAudioEffectManager.h"
#include "EffectPresetReverbApi.h"
#include "EffectVirtualizerApi.h"
#include "EffectBassBoostApi.h"
#include "EffectTrebleBoostApi.h"
#include "EffectEqualizerApi.h"
#include "EffectVolumeApi.h"
...
void CPlayerTestDlg::OnBnClickedOk()
{
	...
	const wchar_t* filename = L"D:\\dump.flv";
...
// 	
	...
	char *pcstr1 = "http://wj.cdnpe.com/live/cc27e411-333e-4b19-a62a-915b8e2992a7_fd5232db-c174-4aad-9a6a-d988d97d93c7.flv";
		//"http://wj.cdnpe.com/live/9e61dd10-a240-400e-8300-ee3531e19fb9_04985f2d-d062-4c11-af2b-decec34dfebd.flv";
		//"http://livesrc.video.taobao.com/live/9e61dd10-a240-400e-8300-ee3531e19fb9_04985f2d-d062-4c11-af2b-decec34dfebd.m3u8?auth_key=1463413829-0-0-b398d99065799d25823eca5d7b9a8157";
...
void CPlayerTestDlg::OnBnClickedExit()
{
	...
	//	effect_uuid_t effectUuidTmp ;
		if (NULL != pInstance)
		{
			if (iVolume != NULL)
			{
				pInstance->RemoveAudioEffect(&VOLUME_UUID_);
			}
			if (iEqualizer != NULL)
			{
				//if (memcpy(&effectUuidTmp, &EQUALIZER_UUID_, sizeof(effect_uuid_t))) 
					//return ;
				pInstance->RemoveAudioEffect(&EQUALIZER_UUID_);
			}
			if (iBassBoost != NULL)
			{
			//	if (memcpy(&effectUuidTmp, &BASSBOOST_UUID_, sizeof(effect_uuid_t))) 
				//	return ;
				pInstance->RemoveAudioEffect(&BASSBOOST_UUID_);
			}
			if(iTrebleBoost!=NULL)
			{
				pInstance->RemoveAudioEffect(&TREBLEBOOST_UUID_);
			}

			if (iReverb != NULL)
			{
				//if (memcpy(&effectUuidTmp, &REVERB_AUX_UUID_, sizeof(effect_uuid_t))) 
					//return ;
				pInstance->RemoveAudioEffect(&REVERB_AUX_UUID_);
				
			}
			if (iVirtualizer != NULL)
			{
			//	if (memcpy(&effectUuidTmp, &VIRTUALIZER_UUID_, sizeof(effect_uuid_t))) 
				//	return ;
				pInstance->RemoveAudioEffect(&VIRTUALIZER_UUID_);
			}
		}
		SAFE_RELEASE(iMediaPlayer);
		OnCancel();
...
void CPlayerTestDlg::OnBnClickedRadioEqPop()
{
	...
	if (iEqualizer != NULL)
	{
		TTInt nParamId = EQ_PARAM_CUR_PRESET;
		TTInt16 aIndex = EQ_Pop;
		iEqualizer->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));
	}
}
...
{
	iVolume = CTTAudioEffectManager::Instance()->QueryAudioEffect(&VOLUME_UUID_);
	if (iVolume == NULL)
	{
		return;
	}
	...
	iVolume->Enable();
	TTInt nParamId = VOLUME_PARAM_LIMIT_ENABLE;
	TTInt32 aEnabled = true;
	iVolume->SetParameters(&nParamId, sizeof(nParamId), &aEnabled, sizeof(aEnabled));
}

void CPlayerTestDlg::InitEq()
{
	iEqualizer = CTTAudioEffectManager::Instance()->QueryAudioEffect(&EQUALIZER_UUID_);
	if (iEqualizer == NULL)
	{
		return;
	}

	//TTInt16 nNumberOfBands = 0;
	TTInt nParamId = EQ_PARAM_NUM_BANDS;
	//TTUint32 nReplySize = sizeof(nNumberOfBands);
	//iEqualizer->GetParameters(&nParamId, sizeof(nParamId), &nNumberOfBands, nReplySize);


	// nParamId = EQ_PARAM_LEVEL_RANGE;
	// nReplySize = sizeof(TTInt16) * 2;
	// iEqualizer->GetParameters(&nParamId, sizeof(nParamId), iLevelRange, nReplySize);

	// TTInt16 nNumberOfPresets;
	// nParamId = EQ_PARAM_GET_NUM_OF_PRESETS;
	// nReplySize = sizeof(nNumberOfPresets);
	// iEqualizer->GetParameters(&nParamId, sizeof(nParamId), &nNumberOfPresets, nReplySize);


	//TTChar cPresetName[32];
	//TTInt nParams[2] = {EQ_PARAM_GET_PRESET_NAME, 10};
	//nReplySize = sizeof(TTChar) * 32;
	//iEqualizer->GetParameters(nParams, sizeof(nParams), cPresetName, nReplySize);

	//nParamId = EQ_PARAM_CUR_PRESET;
	//TTInt16 aIndex = EQ_HeavyMetal ;
	//iEqualizer->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));

	iEqualizer->Enable();

	TTInt16 properties[7];
	properties[0] = -1;
	properties[1] = 5;
	properties[2] = 250;
	properties[3] = 450;
	...
	nParamId = EQ_PARAM_PROPERTIES;
	TTInt16 nValueSize = sizeof(TTInt16) * (2 + properties[1]);

	iEqualizer->SetParameters(&nParamId, sizeof(nParamId), properties, nValueSize);


	//nParamId = EQ_PARAM_PROPERTIES;
	//nReplySize = sizeof(TTInt16) * 7;
	//iEqualizer->GetParameters(&nParamId, sizeof(nParamId), properties, nReplySize);
}
...
void CPlayerTestDlg::SetEqBandLevel(TTInt16 aBand, TTInt aPos)
{
	...
	ASSERT(aBand >= 0 && aBand < nNumberOfBands);

	TTInt16 aLevel = (100 - aPos) * (iLevelRange[1] - iLevelRange[0]) / 100 + iLevelRange[0];
	//TTInt16 aLevel = -12000;
	TTInt nParams[2] = {EQ_PARAM_BAND_LEVEL, aBand};

	iEqualizer->SetParameters(nParams, sizeof(nParams), &aLevel, sizeof(aLevel));

	TTInt16 nLevel;
	//nParams[2] = {EQ_PARAM_BAND_LEVEL, aBand};
	nReplySize = sizeof(nLevel);

	iEqualizer->GetParameters(nParams, sizeof(nParams), &nLevel, nReplySize);

	//return nLevel;


	DisableEqPresetCtrls();
}

void CPlayerTestDlg::InitBassBoost()
{
//	effect_uuid_t bassboostUuid ;

//	if (memcpy(&bassboostUuid, &BASSBOOST_UUID_, sizeof(effect_uuid_t))) 
//		return ;

	iBassBoost = CTTAudioEffectManager::Instance()->QueryAudioEffect(&BASSBOOST_UUID_);
	if (iBassBoost == NULL)
	{
		return;
	}
	...
	TTInt nParamId = BASSBOOST_PARAM_STRENGTH;
	TTInt16 aStrength = 0;
	iBassBoost->SetParameters(&nParamId, sizeof(nParamId), &aStrength, sizeof(aStrength));
	iBassBoost->Disable();
}

void CPlayerTestDlg::InitTrebleBoost()
{
	iTrebleBoost = CTTAudioEffectManager::Instance()->QueryAudioEffect(&TREBLEBOOST_UUID_);
	if (iTrebleBoost == NULL)
	{
		return;
	}
	...
	TTInt nParamId = TREBLEBOOST_PARAM_STRENGTH;
	TTInt16 aStrength = 0;
	iTrebleBoost->SetParameters(&nParamId, sizeof(nParamId), &aStrength, sizeof(aStrength));
	iTrebleBoost->Disable();
}

void CPlayerTestDlg::InitVirtualizer()
{
//	effect_uuid_t virtualizerUuid ;

//	if (memcpy(&virtualizerUuid, &VIRTUALIZER_UUID_, sizeof(effect_uuid_t))) 
//		return ;
	

	iVirtualizer  = CTTAudioEffectManager::Instance()->QueryAudioEffect(&VIRTUALIZER_UUID_);
	if (iVirtualizer == NULL)
	{
		return;
	}
...
//	effect_uuid_t reverbUuid ;

//	if (memcpy(&reverbUuid, &REVERB_AUX_UUID_, sizeof(effect_uuid_t))) 
//		return ;

	iReverb = CTTAudioEffectManager::Instance()->QueryAudioEffect(&REVERB_AUX_UUID_);
	if (iReverb == NULL)
	{
		return;
	}
	...
	TTInt nParamId = REVERB_PARAM_PRESET;
	TTInt16 aIndex = 4 ;
	iReverb->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));
}
...
void CPlayerTestDlg::OnBnClickedCheckBassBoost()
{
	if (iBassBoost != NULL)
	{
		mBassBoostEnable = ((CButton*)GetDlgItem(IDC_CHECK1))->GetCheck();

		mBassBoostEnable ? iBassBoost->Enable() : iBassBoost->Disable();
	}
}
...
void CPlayerTestDlg::OnNMReleasedcaptureBassBoost(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		TTInt nParamId = BASSBOOST_PARAM_STRENGTH;
		TTInt16 aStrength = m_ctrlBassBoost.GetPos();
		iBassBoost->SetParameters(&nParamId, sizeof(nParamId), &aStrength, sizeof(aStrength));
///////////////////////////
		/*	TTInt16 nStrength = 0;
		nParamId = BASSBOOST_PARAM_STRENGTH;
		TTUint32 nReplySize = sizeof(nStrength);

		iBassBoost->GetParameters(&nParamId, sizeof(nParamId), &nStrength, nReplySize);
		int ill =0;*/
...
void CPlayerTestDlg::OnBnClickedCheckVirtualizer()
{
	if (iVirtualizer != NULL)
	{
		mVirtualizerEnable = ((CButton*)GetDlgItem(IDC_CHECK3))->GetCheck();

		mVirtualizerEnable ? iVirtualizer->Enable() : iVirtualizer->Disable();
	}
}
...
void CPlayerTestDlg::OnNMReleasedcaptureVirtualzer(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (iVirtualizer != NULL)
	{
		...
		TTInt16 aStrength = m_ctrlVirtualizer.GetPos();
		TTInt nParamId = VIRTUALIZER_PARAM_STRENGTH;

		iVirtualizer->SetParameters(&nParamId, sizeof(nParamId), &aStrength, sizeof(aStrength));
///////////////////////////
		/*TTInt16 nStrength = 0;
		nParamId = VIRTUALIZER_PARAM_STRENGTH;
		TTUint32 nReplySize = sizeof(nStrength);

		iVirtualizer->GetParameters(&nParamId, sizeof(nParamId), &nStrength, nReplySize);
		TTInt ill= 2;
*/
	}
}
...
void CPlayerTestDlg::OnBnClickedCheckReverb()
{
	if (iReverb != NULL)
	{
		mReverbEnable = ((CButton*)GetDlgItem(IDC_CHECK2))->GetCheck();

		mReverbEnable ? iReverb->Enable() : iReverb->Disable();
	}
}
...
void CPlayerTestDlg::OnBnClickedRadioReverbSmallRoom()
{
	...
	if (iReverb != NULL)
	{
		TTInt16 aIndex = Reverb_SmallRoom ;
		TTInt nParamId = REVERB_PARAM_PRESET;
		iReverb->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));
	}
}
...
void CPlayerTestDlg::OnBnClickedRadioReverbMediumRoom()
{
	...
	if (iReverb != NULL)
	{
		TTInt16 aIndex = Reverb_MediumRoom ;
		TTInt nParamId = REVERB_PARAM_PRESET;
		iReverb->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));
	}
}
...
void CPlayerTestDlg::OnBnClickedRadioReverbLargeRoom()
{
	...
	if (iReverb != NULL)
	{
		TTInt16 aIndex = Reverb_LargeRoom ;
		TTInt nParamId = REVERB_PARAM_PRESET;
		iReverb->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));
	}
}
...
void CPlayerTestDlg::OnBnClickedRadioReverbMediumHall()
{
	...
	if (iReverb != NULL)
	{
		TTInt16 aIndex = Reverb_MediumHall ;
		TTInt nParamId = REVERB_PARAM_PRESET;
		iReverb->SetParameters(&nParamId, sizeof(nParamId), &aIndex, sizeof(aIndex));
	}
}
...
void CPlayerTestDlg::OnBnClickedRadioReverbLargeHall()
{
	...
	if (iReverb != NULL)
	{
		TTInt16 aIndex = Reverb_LargeHall ;
		TTInt nParamId = REVERB_PARAM_PRESET;
...
void CPlayerTestDlg::OnBnClickedCheckTreble()
{
	...
	if (iTrebleBoost != NULL)
	{
		mTrebleBoostEnable = ((CButton*)GetDlgItem(IDC_CHECK_TREBLE))->GetCheck();
		mTrebleBoostEnable ? iTrebleBoost->Enable() : iTrebleBoost->Disable();
	}
...
void CPlayerTestDlg::OnNMReleasedcaptureSldTreble(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		TTInt nParamId = TREBLEBOOST_PARAM_STRENGTH;
		TTInt16 aStrength = m_ctrlTrebleBoost.GetPos();
		iTrebleBoost->SetParameters(&nParamId, sizeof(nParamId), &aStrength, sizeof(aStrength));
	}
}
