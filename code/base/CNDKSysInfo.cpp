/*******************************************************************************
	File:		CNDKSysInfo.cpp

	Contains:	The adnroid ndk system info implement file.
...
void AndroidMemInfo::GetAvailPhys()
{
    ...
    if (m_memFile < 0) 
    {
        QCLOGT ("AndroidMemInfo", "Unable to open /proc/meminfo");
        return;
    }
    ...
    if (len < 0) 
    {
        QCLOGT ("AndroidMemInfo", "Unable to read /proc/meminfo");
        return;
    }
    ...
    while (*p && numFound < 2) 
    {
        ...
        while (sums[i])
        {
            if (strncmp(p, sums[i], sumsLen[i]) == 0) 
            {
                ...
                numFound++;
                //QCLOGI("[ANL]Available %lu, num %s, %lld k", ullAvailPhys, num, atoll(num));
                break;
            }
            i++;
        }
...
void AndroidMemInfo::GetTotalPhys()
{
    ...
    static const int sumsLen[] = { strlen("MemTotal:"), NULL };
    ...
    while (*p ) 
    {
        ...
        while (sums[i])
        {
            if (strncmp(p, sums[i], sumsLen[i]) == 0) 
            {
                ...
                //QCLOGI("[ANL]Total %lu, num %s, %lld k", ullTotalPhys, num, atoll(num));
                break;
            }
            i++;
        }
...
void AndroidCpuUsage::getProcCPUStat(struct systemCPUdata *proc, int pid)
{
    ...
    if (file == NULL)
    {
        QCLOGT ("AndroidCpuUsage", "AndroidCpuUsage Error: process not found!\n");
        return;
    }

    fgets(line, 255, file);
    fclose(file);
    sscanf(line, "%d %s %c %d %d %d %d %d %lu %lu %lu %lu %lu %lu %lu %ld %ld %ld %ld %ld %ld %lu %lu %ld",
...
DWORD AndroidCpuUsage::GetUsedCpu()
{
    ...
    else
    {
        ...
        if (m_nowtime.tv_sec - m_lasttime.tv_sec > 0)
