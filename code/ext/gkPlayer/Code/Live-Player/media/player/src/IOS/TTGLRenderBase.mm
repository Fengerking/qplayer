...
int TTGLRenderBase::initVideoView(bool bReInit)
{
    ...
    if (nil == m_pVideoView) {
        ...
        [m_pVideoView setTouchEnable:mTouchEnable];
    }
...
void TTGLRenderBase::setTouchEnable(bool aEnable)
{
    GKCAutoLock Lock(&mCritical);
    mTouchEnable = aEnable;
    if(nil != m_pVideoView) {
        [m_pVideoView setTouchEnable:aEnable];
    }
    
}

void TTGLRenderBase::setRendType(TTInt aRenderType)
{
    mRenderType = aRenderType;
}

void TTGLRenderBase::setMotionEnable(bool aEnable)
{
...
int TTGLRenderBase::GetLastFrame(void *pData)
{
    return 0;
}
