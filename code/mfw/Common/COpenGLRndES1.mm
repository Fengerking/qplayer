...
bool COpenGLRndES1::UpdateSelfRotate()
{
    if(m_nRotation == COpenGLRnd::QC_ROTATION_0 && m_pBaseInst->m_pSetting->g_qcs_nVideoRotateAngle != 0)
    {
        if (m_pBaseInst->m_pSetting->g_qcs_nVideoRotateAngle == 90)
            m_nRotation = COpenGLRnd::QC_ROTATION_90;
        else if(m_pBaseInst->m_pSetting->g_qcs_nVideoRotateAngle == 270)
        {
            m_nRotation = COpenGLRnd::QC_ROTATION_270;
        }
    ...
    else if(m_nRotation != COpenGLRnd::QC_ROTATION_0 && m_pBaseInst->m_pSetting->g_qcs_nVideoRotateAngle == 0)
    {
        m_nRotation = COpenGLRnd::QC_ROTATION_0;
        return true;
    }
...
int COpenGLRndES1::UploadTexture(QC_VIDEO_BUFF* pVideoBuffer)
{
    //if(pVideoBuffer->nType == QC_VDT_YUV420_P)
    {
        ...
        if(pVideoBuffer->nType == QC_VDT_YUV420_P)
        {
            QC_VIDEO_BUFF bufRender;
            bufRender.nType = (m_nRGBType==565?QC_VDT_RGB565:QC_VDT_ARGB);
            bufRender.nWidth = m_nInputWidth;
            bufRender.nHeight = m_nInputHeight;
            bufRender.pBuff[0] = m_pFrameData;
            bufRender.nStride[0] = nRndStride;
            if (m_fColorCvtR != NULL)
                m_fColorCvtR(pVideoBuffer, &bufRender, 0);
        }
        else if(pVideoBuffer->nType == QC_VDT_NV12)
        {
            ...
            QC_VIDEO_BUFF bufRender;
            bufRender.nType = (m_nRGBType==565?QC_VDT_RGB565:QC_VDT_ARGB);
            bufRender.nWidth = m_nInputHeight;
            bufRender.nHeight = m_nInputWidth;
//            static unsigned char* pFrameData = new unsigned char[nRndStride * m_nTextureHeight];
//            bufRender.pBuff[0] = pFrameData;
            bufRender.pBuff[0] = m_pFrameData;
            bufRender.nStride[0] = nRndStride;
            
            if (m_fColorCvtR != NULL)
            {
                CVPixelBufferRef pixelBuffer = (__bridge CVPixelBufferRef)pVideoBuffer->pBuff[0];
                CVReturn nCVRet = CVPixelBufferLockBaseAddress(pixelBuffer, 0);
                if (nCVRet == kCVReturnSuccess)
                {
                    QC_VIDEO_BUFF inputBuf;
                    if (CVPixelBufferIsPlanar(pixelBuffer))
                    {
                        size_t count = CVPixelBufferGetPlaneCount(pixelBuffer);
                        for (size_t i = 0; i < count; i++)
                        {
                            inputBuf.nStride[i]   = (int)CVPixelBufferGetBytesPerRowOfPlane(pixelBuffer, i);
                            inputBuf.pBuff[i]     = (unsigned char *)CVPixelBufferGetBaseAddressOfPlane(pixelBuffer, i);
                        }
                        inputBuf.nWidth = pVideoBuffer->nWidth;
                        inputBuf.nHeight = pVideoBuffer->nHeight;
                        inputBuf.nType = pVideoBuffer->nType;
                        m_fColorCvtR(&inputBuf, &bufRender, 0);
                        
//                        QC_VIDEO_BUFF bufRender2;
//                        bufRender2.nType = (m_nRGBType==565?QC_VDT_RGB565:QC_VDT_ARGB);
//                        bufRender2.nWidth = m_nInputWidth;
//                        bufRender2.nHeight = m_nInputHeight;
//                        bufRender2.pBuff[0] = m_pFrameData;
//                        bufRender2.nStride[0] = nRndStride;
//                        m_fColorCvtR(&bufRender, &bufRender2, 90);
                    }
                }
...
//    GLfloat nYTop = 1 - ((GLfloat)(2 * m_rcGLFrameDrawArea.origin.y) / m_nBackingHeight); // from top
    ...
    switch (m_nRotation)
    {
        ...
        case COpenGLRnd::QC_ROTATION_90:  //4231
        ...
        case COpenGLRnd::QC_ROTATION_270:  //1324
...
//        case COpenGLRnd::ROTATION_90FLIP:  //3142
...
//        case COpenGLRnd::ROTATION_270FLIP: //2413
...
//    NSLog(@"point(%f, %f), m_fVertices: %f, %f, %f, %f, %f, %f, %f, %f \n\n",
