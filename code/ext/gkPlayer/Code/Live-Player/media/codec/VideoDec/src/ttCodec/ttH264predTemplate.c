                                    ...
                                    ptrdiff_t _stride)
{
    ...
    const pixel4 a= TTV_RN4PA(src-stride);

    TTV_WN4PA(src+0*stride, a);
    TTV_WN4PA(src+1*stride, a);
    TTV_WN4PA(src+2*stride, a);
    TTV_WN4PA(src+3*stride, a);
}
                                      ...
                                      ptrdiff_t _stride)
{
    pixel *src = (pixel*)_src;
    int stride = _stride>>(sizeof(pixel)-1);
    TTV_WN4PA(src+0*stride, PIXEL_SPLAT_X4(src[-1+0*stride]));
    TTV_WN4PA(src+1*stride, PIXEL_SPLAT_X4(src[-1+1*stride]));
    TTV_WN4PA(src+2*stride, PIXEL_SPLAT_X4(src[-1+2*stride]));
    TTV_WN4PA(src+3*stride, PIXEL_SPLAT_X4(src[-1+3*stride]));
}
...
#define LOAD_TOP_RIGHT_EDGE\
    const unsigned ttv_unused t4 = topright[0];\
    const unsigned ttv_unused t5 = topright[1];\
    const unsigned ttv_unused t6 = topright[2];\
    const unsigned ttv_unused t7 = topright[3];\

#define LOAD_DOWN_LEFT_EDGE\
    const unsigned ttv_unused l4 = src[-1+4*stride];\
    const unsigned ttv_unused l5 = src[-1+5*stride];\
    const unsigned ttv_unused l6 = src[-1+6*stride];\
    const unsigned ttv_unused l7 = src[-1+7*stride];\

#define LOAD_LEFT_EDGE\
    const unsigned ttv_unused l0 = src[-1+0*stride];\
    const unsigned ttv_unused l1 = src[-1+1*stride];\
    const unsigned ttv_unused l2 = src[-1+2*stride];\
    const unsigned ttv_unused l3 = src[-1+3*stride];\

#define LOAD_TOP_EDGE\
    const unsigned ttv_unused t0 = src[ 0-1*stride];\
    const unsigned ttv_unused t1 = src[ 1-1*stride];\
    const unsigned ttv_unused t2 = src[ 2-1*stride];\
    const unsigned ttv_unused t3 = src[ 3-1*stride];\
...
static void FUNCC(pred16x16_vertical)(uint8_t *_src, ptrdiff_t _stride)
{
    ...
    pixel *src = (pixel*)_src;
    int stride = _stride>>(sizeof(pixel)-1);
    const pixel4 a = TTV_RN4PA(((pixel4*)(src-stride))+0);
    const pixel4 b = TTV_RN4PA(((pixel4*)(src-stride))+1);
    const pixel4 c = TTV_RN4PA(((pixel4*)(src-stride))+2);
    const pixel4 d = TTV_RN4PA(((pixel4*)(src-stride))+3);

    for(i=0; i<16; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, a);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, b);
        TTV_WN4PA(((pixel4*)(src+i*stride))+2, c);
        TTV_WN4PA(((pixel4*)(src+i*stride))+3, d);
    }
}
...
static void FUNCC(pred16x16_horizontal)(uint8_t *_src, ptrdiff_t stride)
{
    ...
    for(i=0; i<16; i++){
        ...
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, a);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, a);
        TTV_WN4PA(((pixel4*)(src+i*stride))+2, a);
        TTV_WN4PA(((pixel4*)(src+i*stride))+3, a);
    }
}
...
static void FUNCC(pred8x8_left_dc)(uint8_t *_src, ptrdiff_t stride)
{
    ...
    for(i=0; i<4; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc0splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc0splat);
    }
    for(i=4; i<8; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc2splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc2splat);
    }
}
...
static void FUNCC(pred8x8_top_dc)(uint8_t *_src, ptrdiff_t stride)
{
    ...
    for(i=0; i<4; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc0splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc1splat);
    }
    for(i=4; i<8; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc0splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc1splat);
...
static void FUNCC(pred8x8_dc)(uint8_t *_src, ptrdiff_t stride)
{
    ...
    for(i=4; i<8; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc2splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc3splat);
    }
}
...
static void FUNCC(pred8x16_dc)(uint8_t *_src, ptrdiff_t stride)
{
    ...
    for(i=8; i<12; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc4splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc5splat);
    }
    for(i=12; i<16; i++){
        TTV_WN4PA(((pixel4*)(src+i*stride))+0, dc6splat);
        TTV_WN4PA(((pixel4*)(src+i*stride))+1, dc7splat);
    }
}
...
#define PREDICT_8x8_LOAD_LEFT \
    ...
    const int l7 ttv_unused = (SRC(-1,6) + 3*SRC(-1,7) + 2) >> 2
...
#define PREDICT_8x8_LOAD_TOP \
    ...
    const int t7 ttv_unused = ((has_topright ? SRC(8,-1) : SRC(7,-1)) \
...
#define PREDICT_8x8_DC(v) \
    int y; \
    for( y = 0; y < 8; y++ ) { \
        TTV_WN4PA(((pixel4*)src)+0, v); \
        TTV_WN4PA(((pixel4*)src)+1, v); \
        src += stride; \
    }
...
#define ROW(y) a = PIXEL_SPLAT_X4(l##y); \
               TTV_WN4PA(src+y*stride, a); \
               TTV_WN4PA(src+y*stride+4, a);
                                     ...
                                     int has_topright, ptrdiff_t _stride)
{
    ...
    src[6] = t6;
    src[7] = t7;
    a = TTV_RN4PA(((pixel4*)src)+0);
    b = TTV_RN4PA(((pixel4*)src)+1);
    for( y = 1; y < 8; y++ ) {
        TTV_WN4PA(((pixel4*)(src+y*stride))+0, a);
        TTV_WN4PA(((pixel4*)(src+y*stride))+1, b);
    }
}
