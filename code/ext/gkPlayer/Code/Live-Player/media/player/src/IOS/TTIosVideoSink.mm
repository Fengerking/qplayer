...
CTTIosVideoSink::CTTIosVideoSink(CTTSrcDemux* aSrcMux, TTCBaseAudioSink* aAudioSink, GKDecoderType aDecoderType)
...
TTInt CTTIosVideoSink::render()
{
	...
	GKCAutoLock Lock(&mCriView);
    
    if(mSinkBuffer.Buffer[0] == NULL)
        return TTKErrArgument;
    
    //LOGE("    Video.time = %lld",mSinkBuffer.Time);
    /*int i=0, a=0;
    char* lpSurf = lpSurf1;
    if(lpSurf) {
...
TTInt CTTIosVideoSink::newVideoView()
{
    ...
    if (NULL == m_pRender) {
        if(m_Context) {
            EAGLContext* pContext = (EAGLContext*)m_Context;
            [pContext release];
            m_Context = NULL;
        }
        ...
        m_pRender->setMotionEnable(mMotionEnable);
        m_pRender->setRendType(mRenderType);
        m_pRender->SetView((UIView*)mView);
        m_pRender->init();
            
        [pool release];
            
        m_Context = (void*)pContext;
    }
...
{
	if(mVideoFormat.Width == 0 || mVideoFormat.Height == 0 || mSinkBuffer.Stride[0] == 0)	{
		return TTKErrNotReady;
	}
    
    GKCAutoLock Lock(&mCriView);
    
    if (m_pRender == NULL || mView == NULL || mSinkBuffer.Buffer[0] == NULL) {
        return TTKErrNotReady;
    }
...
void CTTIosVideoSink::setRendType(TTInt aRenderType)
{
...
void CTTIosVideoSink::setMotionEnable(bool aEnable)
{
...
void CTTIosVideoSink::setTouchEnable(bool aEnable)
{
    mTouchEnable = aEnable;
    GKCAutoLock Lock(&mCriView);
    if(m_pRender) {
        m_pRender->setTouchEnable(mTouchEnable);
    }
}
