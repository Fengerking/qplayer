...
TTInt CTTCacheBuffer::Read(TTUint8* aBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
...
TTInt CTTCacheBuffer::Write(TTUint8* aBuffer, TTInt64 aWritePos, TTInt aWriteSize)
{
    GKASSERT((aBuffer != NULL) && (aWriteSize > 0));

	TTInt64 nWriteSize = 0;
	TTInt nIndex = 0;
	GKCAutoLock Lock(&iCritical);
	nIndex = iIndexEnd%PAGE_BUFFER_COUNT;	
