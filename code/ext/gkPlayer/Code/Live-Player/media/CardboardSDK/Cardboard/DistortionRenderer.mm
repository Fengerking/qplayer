...
DistortionRenderer::DistortionRenderer() :
    ...
    _chromaticAberrationCorrectionEnabled(true),
    _vignetteEnabled(true),
    _leftEyeDistortionMesh(NULL),
    _rightEyeDistortionMesh(NULL),
    _glStateBackup(NULL),
    _glStateBackupAberration(NULL),
    _headMountedDisplay(NULL),
    _leftEyeViewport(NULL),
    _rightEyeViewport(NULL),
    _fovsChanged(false),
...
DistortionRenderer::~DistortionRenderer()
{
    if (_glStateBackup != NULL) { delete _glStateBackup; }
    if (_glStateBackupAberration != NULL) { delete _glStateBackupAberration; }
    
    if (_leftEyeDistortionMesh != NULL) { delete _leftEyeDistortionMesh; }
    if (_rightEyeDistortionMesh != NULL) { delete _rightEyeDistortionMesh; }
        
    if (_leftEyeViewport != NULL) { delete _leftEyeViewport; }
    if (_rightEyeViewport != NULL) { delete _rightEyeViewport; }
    
    if (_programHolder != NULL) { delete _programHolder; }
    if (_programHolderAberration != NULL) { delete _programHolderAberration; }
}
...
void DistortionRenderer::beforeDrawFrame()
{
    ...
    //glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, _textureWidthPx, _textureHeightPx);
    //GLCheckForError();
...
void DistortionRenderer::updateTextureAndDistortionMesh()
{
    ...
    if (_programHolder == NULL)
    {
        _programHolder = createProgramHolder(false);
    }
    if (_programHolderAberration == NULL)
    {
    ...
    _textureWidthPx = textureWidthPx;
    _textureHeightPx = textureHeightPx;
...
GLuint DistortionRenderer::getTextID()
{
    return _textureID;
}
