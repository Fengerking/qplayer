...
#define op_scale1(x)  block[x] = ttv_clip_pixel( (block[x]*weight + offset) >> log2_denom )
#define op_scale2(x)  dst[x] = ttv_clip_pixel( (src[x]*weights + dst[x]*weightd + offset) >> (log2_denom+1))
...
static ttv_always_inline ttv_flatten void FUNCC(h264_loop_filter_luma)(uint8_t *p_pix, int xstride, int ystride, int inner_iters, int alpha, int beta, int8_t *tc0)
{
    ...
    for( i = 0; i < 4; i++ ) {
        ...
        for( d = 0; d < inner_iters; d++ ) {
            ...
            if( FFABS( p0 - q0 ) < alpha &&
                ...
                if( FFABS( p2 - p0 ) < beta ) {
                    if(tc_orig)
                    pix[-2*xstride] = p1 + ttv_clip( (( p2 + ( ( p0 + q0 + 1 ) >> 1 ) ) >> 1) - p1, -tc_orig, tc_orig );
                    tc++;
                }
                if( FFABS( q2 - q0 ) < beta ) {
                    if(tc_orig)
                    pix[   xstride] = q1 + ttv_clip( (( q2 + ( ( p0 + q0 + 1 ) >> 1 ) ) >> 1) - q1, -tc_orig, tc_orig );
                    tc++;
                }

                i_delta = ttv_clip( (((q0 - p0 ) << 2) + (p1 - q1) + 4) >> 3, -tc, tc );
                pix[-xstride] = ttv_clip_pixel( p0 + i_delta );    /* p0' */
                pix[0]        = ttv_clip_pixel( q0 - i_delta );    /* q0' */
            }
            pix += ystride;
        }
    }
...
static ttv_always_inline ttv_flatten void FUNCC(h264_loop_filter_luma_intra)(uint8_t *p_pix, int xstride, int ystride, int inner_iters, int alpha, int beta)
{
    pixel *pix = (pixel*)p_pix;
    int d;
    xstride >>= sizeof(pixel)-1;
...
static void FUNCC(h264_h_loop_filter_luma_mbatt_intra)(uint8_t *pix, int stride, int alpha, int beta)
{
    FUNCC(h264_loop_filter_luma_intra)(pix, sizeof(pixel), stride, 2, alpha, beta);
}

static ttv_always_inline ttv_flatten void FUNCC(h264_loop_filter_chroma)(uint8_t *p_pix, int xstride, int ystride, int inner_iters, int alpha, int beta, int8_t *tc0)
{
    pixel *pix = (pixel*)p_pix;
    int i, d;
    alpha <<= BIT_DEPTH - 8;
    ...
    for( i = 0; i < 4; i++ ) {
        ...
        for( d = 0; d < inner_iters; d++ ) {
            ...
            if( FFABS( p0 - q0 ) < alpha &&
                ...
                int delta = ttv_clip( (((q0 - p0 ) << 2) + (p1 - q1) + 4) >> 3, -tc, tc );

                pix[-xstride] = ttv_clip_pixel( p0 + delta );    /* p0' */
                pix[0]        = ttv_clip_pixel( q0 - delta );    /* q0' */
            }
            pix += ystride;
        }
    }
...
static ttv_always_inline ttv_flatten void FUNCC(h264_loop_filter_chroma_intra)(uint8_t *p_pix, int xstride, int ystride, int inner_iters, int alpha, int beta)
{
    pixel *pix = (pixel*)p_pix;
    int d;
    xstride >>= sizeof(pixel)-1;
...
static void FUNCC(h264_h_loop_filter_chroma_mbatt_intra)(uint8_t *pix, int stride, int alpha, int beta)
{
...
static void FUNCC(h264_h_loop_filter_chroma422_mbatt_intra)(uint8_t *pix, int stride, int alpha, int beta)
{
    FUNCC(h264_loop_filter_chroma_intra)(pix, sizeof(pixel), stride, 2, alpha, beta);
}
