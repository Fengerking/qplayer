...
GetDHOffset2(uint8_t *handshake, unsigned int len)
{
  ...
  if (res + 128 > 767)
    {
      LOGE("GetDHOffset2: Couldn't calculate correct DH offset (got %d), exiting!", res);
      exit(1);
    }
  return res;
}
...
GetDigestOffset2(uint8_t *handshake, unsigned int len)
{
  ...
  if (res + 32 > 1535)
    {
      LOGE("GetDigestOffset2: Couldn't calculate correct digest offset (got %d), exiting", res);
      exit(1);
    }
  return res;
}
...
GetDHOffset1(uint8_t *handshake, unsigned int len)
{
  ...
  if (res + 128 > 1531)
    {
      LOGE("GetDigestOffset1: Couldn't calculate DH offset (got %d), exiting!", res);
      exit(1);
    }
...
GetDigestOffset1(uint8_t *handshake, unsigned int len)
{
  ...
  if (res + 32 > 771)
    {
      LOGE("GetDigestOffset1: Couldn't calculate digest offset (got %d), exiting!", res);
      exit(1);
    }
...
HandShake(RTMP * r, int FP9HandShake)
{
  ...
  if (FP9HandShake)
      ...
      if (encrypted)
	{
      ...
      LOGI("HandShake: Client type: %02X", clientsig[-1]);
  ...
  if (FP9HandShake)
      ...
      if (encrypted)
	{
	  ...
	  if (!r->Link.dh)
	    {
	      LOGE("HandShake: Couldn't initialize Diffie-Hellmann!");
	      return FALSE;
	    }

	  dhposClient = getdh(clientsig, RTMP_SIG_SIZE);
	  LOGI("HandShake: DH pubkey position: %d", dhposClient);

	  if (!DHGenerateKey(r->Link.dh))
	    {
	      LOGE("HandShake: Couldn't generate Diffie-Hellmann public key!");
	      return FALSE;
	    }

	  if (!DHGetPublicKey(r->Link.dh, &clientsig[dhposClient], 128))
	    {
	      LOGE("HandShake: Couldn't write public key!");
	      return FALSE;
	    }
	}

      digestPosClient = getdig(clientsig, RTMP_SIG_SIZE);	/* reuse this value in verification */
      LOGI("HandShake: Client digest offset: %d",digestPosClient);

      CalculateDigest(digestPosClient, clientsig, GenuineFPKey, 30,
		      &clientsig[digestPosClient]);

      LOGI("HandShake: Initial client digest: ");
  ...
  if (type != clientsig[-1])
    LOGI("HandShake: Type mismatch: client sent %d, server answered %d", clientsig[-1], type);
  ...
  LOGI("HandShake: Server Uptime : %d", uptime);
  LOGI("HandShake: FMS Version   : %d.%d.%d.%d", serversig[4],
  ...
  if (FP9HandShake)
	...
	{
	  LOGI("Trying different position for server digest!");
	  offalg ^= 1;
	  getdig = digoff[offalg];
	  getdh  = dhoff[offalg];
      ...
      if (encrypted)
	{
	  ...
	  LOGI("HandShake: Server DH public key offset: %d", dhposServer);
	  len = DHComputeSharedSecretKey(r->Link.dh, &serversig[dhposServer],
	  				128, secretKey);
	  if (len < 0)
	    {
	      LOGI("HandShake: Wrong secret key position!");
	      return FALSE;
	    }

	  LOGI("HandShake: Secret key: ");
      ...
      /* some info output */
      LOGI("HandShake: Calculated digest key from secure key and server digest: ");
...
#endif
#endif
      LOGI("HandShake: Client signature calculated:");
...
#ifdef _DEBUG
  LOGI("HandShake: Sending handshake response: ");
  ...
  if (FP9HandShake)
    {
	  ...
	  && serversig[7] == 0)
	{
	  LOGI("HandShake: Wait, did the server just refuse signed authentication?");
	}
      LOGI("HandShake: Server sent signature:");
...
#endif
#endif
      LOGI("HandShake: Signature calculated:");
      RTMP_LogHex(RTMP_LOGDEBUG, signature, SHA256_DIGEST_LENGTH);
      if (memcmp
	  (signature, &serversig[RTMP_SIG_SIZE - SHA256_DIGEST_LENGTH],
	   SHA256_DIGEST_LENGTH) != 0)
	{
	  LOGI("HandShake: Server not genuine Adobe!");
	  return FALSE;
	}
      else
	{
	  LOGI("HandShake: Genuine Adobe Flash Media Server");
	}
...
SHandShake(RTMP * r)
{
  ...
  LOGI("SHandShake: Type Requested : %02X", type);
  ...
  if (type == 3)
    ...
    {
      LOGE("SHandShake: Unknown version %02x", type);
      return FALSE;
    }
  ...
  if (FP9HandShake)
      ...
      if (encrypted)
	{
	  ...
	  if (!r->Link.dh)
	    {
	      LOGE("SHandShake: Couldn't initialize Diffie-Hellmann!");
	      return FALSE;
	    }

	  dhposServer = getdh(serversig, RTMP_SIG_SIZE);
	  LOGI("SHandShake: DH pubkey position: %d", dhposServer);

	  if (!DHGenerateKey(r->Link.dh))
	    {
	      LOGE("SHandShake: Couldn't generate Diffie-Hellmann public key!");
	      return FALSE;
	    }
  ...
  LOGI("SHandShake: Client Uptime : %d", uptime);
  LOGI("SHandShake: Player Version: %d.%d.%d.%d", clientsig[4],
  ...
  if (FP9HandShake)
	...
	{
	  LOGI("Trying different position for client digest!");
	  offalg ^= 1;
	  getdig = digoff[offalg];
	  getdh  = dhoff[offalg];
	  ...
	  if (!VerifyDigest(digestPosClient, clientsig, GenuineFPKey, 30))
	    {
	      LOGE("Couldn't verify the client digest");	/* continuing anyway will probably fail */
	      return FALSE;
	    }
	}
      ...
      if (encrypted)
	{
	  ...
	  dhposClient = getdh(clientsig, RTMP_SIG_SIZE);
	  LOGI("SHandShake: Client DH public key offset: %d", dhposClient);
	  len =
	  ...
	  if (len < 0)
	    {
	      LOGI("SHandShake: Wrong secret key position!");
	      return FALSE;
	    }

	  LOGI("SHandShake: Secret key: ");
	  RTMP_LogHex(RTMP_LOGDEBUG, secretKey, 128);
      ...
      /* some info output */
      LOGI("SHandShake: Calculated digest key from secure key and server digest: ");
      RTMP_LogHex(RTMP_LOGDEBUG, digestResp, SHA256_DIGEST_LENGTH);

      LOGI("SHandShake: Server signature calculated:");
      RTMP_LogHex(RTMP_LOGDEBUG, signatureResp, SHA256_DIGEST_LENGTH);
    }
#if 0
  else
  ...
  LOGI("SHandShake: Sending handshake response: ");
  RTMP_LogHex(RTMP_LOGDEBUG2, clientsig, RTMP_SIG_SIZE);
  ...
  if (FP9HandShake)
    {
      ...
      LOGI("SHandShake: Client sent signature:");
      ...
      /* show some information */
      LOGI("SHandShake: Digest key: ");
      RTMP_LogHex(RTMP_LOGDEBUG, digest, SHA256_DIGEST_LENGTH);

      LOGI("SHandShake: Signature calculated:");
      RTMP_LogHex(RTMP_LOGDEBUG, signature, SHA256_DIGEST_LENGTH);
      if (memcmp
	  (signature, &clientsig[RTMP_SIG_SIZE - SHA256_DIGEST_LENGTH],
	   SHA256_DIGEST_LENGTH) != 0)
	{
	  LOGI("SHandShake: Client not genuine Adobe!");
	  return FALSE;
	}
      else
	{
	  LOGI("SHandShake: Genuine Adobe Flash Player");
	}
  ...
  LOGI("SHandShake: Handshaking finished....");
  return TRUE;
}
