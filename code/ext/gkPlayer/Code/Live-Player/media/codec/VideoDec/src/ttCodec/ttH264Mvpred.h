#ifndef __TTPOD_TT_H264_MVPRED_H
#define __TTPOD_TT_H264_MVPRED_H
...
static ttv_always_inline int fetch_diagonal_mv(H264Context *h, const int16_t **C,
...
#define SET_DIAG_MV(MV_OP, REF_OP, XY, Y4)                              \
        ...
        if (topright_ref == PART_NOT_AVAILABLE
            ...
            const int16_t *mv;
            TTV_ZERO32(h->mv_cache[list][scan8[0] - 2]);
 ...
 */
static ttv_always_inline void pred_motion(H264Context *const h, int n,
                                         ...
                                         int *const mx, int *const my)
{
    ...
    ttv_assert2(part_width == 1 || part_width == 2 || part_width == 4);
 ...
 * @param my the y component of the predicted motion vector
 */
static ttv_always_inline void pred_16x8_motion(H264Context *const h,
 ...
 * @param my the y component of the predicted motion vector
 */
static ttv_always_inline void pred_8x16_motion(H264Context *const h,
                                              int n, int list, int ref,
                                              int *const mx, int *const my)
{
    if (n == 0) {
...
static ttv_always_inline void pred_pskip_motion(H264Context *const h)
{
...
static void fill_decode_caches(H264Context *h, int mb_type)
{
    ...
    if (!IS_SKIP(mb_type)) {
        if (IS_INTRA(mb_type)) {
            ...
            if (IS_INTERLACED(mb_type) != IS_INTERLACED(left_type[LTOP])) {
                if (IS_INTERLACED(mb_type)) {
                    ...
                    ttv_assert2(left_xy[LTOP] == left_xy[LBOT]);
            ...
            if (IS_INTRA4x4(mb_type)) {
                if (IS_INTRA4x4(top_type)) {
                    TTV_COPY32(h->intra4x4_pred_mode_cache + 4 + 8 * 0, h->intra4x4_pred_mode + h->mb2br_xy[top_xy]);
                } else {
        ...
        if (top_type) {
            ...
            TTV_COPY32(&nnz_cache[4 + 8 * 0], &nnz[4 * 3]);
            if (!h->chroma_y_shift) {
                TTV_COPY32(&nnz_cache[4 + 8 *  5], &nnz[4 *  7]);
                TTV_COPY32(&nnz_cache[4 + 8 * 10], &nnz[4 * 11]);
            } else {
                TTV_COPY32(&nnz_cache[4 + 8 *  5], &nnz[4 * 5]);
                TTV_COPY32(&nnz_cache[4 + 8 * 10], &nnz[4 * 9]);
            }
        } else {
            uint32_t top_empty = CABAC(h) && !IS_INTRA(mb_type) ? 0 : 0x40404040;
            TTV_WN32A(&nnz_cache[4 + 8 *  0], top_empty);
            TTV_WN32A(&nnz_cache[4 + 8 *  5], top_empty);
            TTV_WN32A(&nnz_cache[4 + 8 * 10], top_empty);
        }
    ...
    if (IS_INTER(mb_type) || (IS_DIRECT(mb_type) && h->direct_spatial_mv_pred)) {
        ...
        for (list = 0; list < h->list_count; list++) {
            ...
            if (!USES_LIST(mb_type, list))
                continue;
            ttv_assert2(!(IS_DIRECT(mb_type) && !h->direct_spatial_mv_pred));

            if (USES_LIST(top_type, list)) {
                const int b_xy = h->mb2b_xy[top_xy] + 3 * b_stride;
                TTV_COPY128(mv_cache[0 - 1 * 8], mv[b_xy + 0]);
                ref_cache[0 - 1 * 8] =
                ref_cache[1 - 1 * 8] = ref[4 * top_xy + 2];
                ref_cache[2 - 1 * 8] =
                ref_cache[3 - 1 * 8] = ref[4 * top_xy + 3];
            } else {
                TTV_ZERO128(mv_cache[0 - 1 * 8]);
                TTV_WN32A(&ref_cache[0 - 1 * 8],
            ...
            if (mb_type & (MB_TYPE_16x8 | MB_TYPE_8x8)) {
                for (i = 0; i < 2; i++) {
                    ...
                    if (USES_LIST(left_type[LEFT(i)], list)) {
                        ...
                        TTV_COPY32(mv_cache[cache_idx],
                                  mv[b_xy + b_stride * left_block[0 + i * 2]]);
                        TTV_COPY32(mv_cache[cache_idx + 8],
                                  mv[b_xy + b_stride * left_block[1 + i * 2]]);
                        ref_cache[cache_idx]     = ref[b8_xy + (left_block[0 + i * 2] & ~1)];
                        ref_cache[cache_idx + 8] = ref[b8_xy + (left_block[1 + i * 2] & ~1)];
                    } else {
                        TTV_ZERO32(mv_cache[cache_idx]);
                        TTV_ZERO32(mv_cache[cache_idx + 8]);
                        ref_cache[cache_idx]     =
                ...
                if (USES_LIST(left_type[LTOP], list)) {
                    ...
                    TTV_COPY32(mv_cache[-1], mv[b_xy + b_stride * left_block[0]]);
                    ref_cache[-1] = ref[b8_xy + (left_block[0] & ~1)];
                } else {
                    TTV_ZERO32(mv_cache[-1]);
            ...
            if (USES_LIST(topright_type, list)) {
                ...
                TTV_COPY32(mv_cache[4 - 1 * 8], mv[b_xy]);
                ref_cache[4 - 1 * 8] = ref[4 * topright_xy + 2];
            } else {
                TTV_ZERO32(mv_cache[4 - 1 * 8]);
            ...
            if(ref_cache[2 - 1*8] < 0 || ref_cache[4 - 1 * 8] < 0) {
                if (USES_LIST(topleft_type, list)) {
                    ...
                    TTV_COPY32(mv_cache[-1 - 1 * 8], mv[b_xy]);
                    ref_cache[-1 - 1 * 8] = ref[b8_xy];
                } else {
                    TTV_ZERO32(mv_cache[-1 - 1 * 8]);
            ...
            if (!(mb_type & (MB_TYPE_SKIP | MB_TYPE_DIRECT2))) {
                ...
                TTV_ZERO32(mv_cache[2 + 8 * 0]);
                TTV_ZERO32(mv_cache[2 + 8 * 2]);

                if (CABAC(h)) {
                    if (USES_LIST(top_type, list)) {
                        const int b_xy = h->mb2br_xy[top_xy];
                        TTV_COPY64(mvd_cache[0 - 1 * 8], mvd[b_xy + 0]);
                    } else {
                        TTV_ZERO64(mvd_cache[0 - 1 * 8]);
                    }
                    if (USES_LIST(left_type[LTOP], list)) {
                        const int b_xy = h->mb2br_xy[left_xy[LTOP]] + 6;
                        TTV_COPY16(mvd_cache[-1 + 0 * 8], mvd[b_xy - left_block[0]]);
                        TTV_COPY16(mvd_cache[-1 + 1 * 8], mvd[b_xy - left_block[1]]);
                    } else {
                        TTV_ZERO16(mvd_cache[-1 + 0 * 8]);
                        TTV_ZERO16(mvd_cache[-1 + 1 * 8]);
                    }
                    if (USES_LIST(left_type[LBOT], list)) {
                        const int b_xy = h->mb2br_xy[left_xy[LBOT]] + 6;
                        TTV_COPY16(mvd_cache[-1 + 2 * 8], mvd[b_xy - left_block[2]]);
                        TTV_COPY16(mvd_cache[-1 + 3 * 8], mvd[b_xy - left_block[3]]);
                    } else {
                        TTV_ZERO16(mvd_cache[-1 + 2 * 8]);
                        TTV_ZERO16(mvd_cache[-1 + 3 * 8]);
                    }
                    TTV_ZERO16(mvd_cache[2 + 8 * 0]);
                    TTV_ZERO16(mvd_cache[2 + 8 * 2]);
 ...
 */
static void ttv_unused decode_mb_skip(H264Context *h)
{
    const int mb_xy = h->mb_xy;
    int mb_type     = 0;
...
#endif /* __TTPOD_TT_H264_MVPRED_H */
