    ...
    if (IS_16X16(mb_type)) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (IS_SUB_8X8(sub_mb_type)) {
                ...
                int j;
                ttv_assert2(IS_SUB_4X4(sub_mb_type));
                for (j = 0; j < 4; j++) {
        ...
        for (ref = 0; ref < 48 && nrefs[list]; ref++) {
            ...
            if (row >= 0) {
                ...
                if (!FIELD_PICTURE(h) && ref_field_picture) { // frame referencing two fields
                    tt_thread_await_progress(&ref_pic->tf,
                                             FFMIN((row >> 1) - !(row & 1),
                                                   pic_height - 1),
                                             1);
                    tt_thread_await_progress(&ref_pic->tf,
...
static ttv_always_inline void mc_dir_part(H264Context *h, H264Picture *pic,
...
static ttv_always_inline void mc_part_std(H264Context *h, int n, int square,
...
static ttv_always_inline void mc_part_weighted(H264Context *h, int n, int square,
                                              int height, int delta,
                                              uint8_t *dest_y, uint8_t *dest_cb,
                                              uint8_t *dest_cr,
                                              int x_offset, int y_offset,
...
static ttv_always_inline void prefetch_motion(H264Context *h, int list,
                                             int pixel_shift, int chroma_idc)
{
...
static ttv_always_inline void xchg_mb_border(H264Context *h, uint8_t *src_y,
...
static ttv_always_inline int dctcoef_get(int16_t *mb, int high_bit_depth,
                                        int index)
{
    if (high_bit_depth) {
        return TTV_RN32A(((int32_t *)mb) + index);
    } else
        return TTV_RN16A(mb + index);
}

static ttv_always_inline void dctcoef_set(int16_t *mb, int high_bit_depth,
                                         int index, int value)
{
    if (high_bit_depth) {
        TTV_WN32A(((int32_t *)mb) + index, value);
    } else
        TTV_WN16A(mb + index, value);
}

static ttv_always_inline void hl_decode_mb_predict_luma(H264Context *h,
    ...
    if (IS_INTRA4x4(mb_type)) {
            ...
            for (i = 0; i < 16; i++) {
                ...
                if (transform_bypass && h->sps.profile_idc == 244 && dir <= 1) {
                    ...
                    if (dir == DIAG_DOWN_LEFT_PRED || dir == VERT_LEFT_PRED) {
                        ...
                        ttv_assert2(h->mb_y || linesize <= block_offset[i]);
                        if (!topright_avail) {
                            if (pixel_shift) {
                    ...
                    if (nnz) {
                        if (is_h264) {
                            if (nnz == 1 && dctcoef_get(h->mb, pixel_shift, i * 16 + p * 256))
                             ...
                             tt_svq3_add_idct_c(ptr, h->mb + i * 16 + p * 256, linesize, qscale, 0);
#endif
                    }
                }
            }
        ...
        if (is_h264) {
            if (h->non_zero_count_cache[scan8[LUMA_DC_BLOCK_INDEX + p]]) {
             ...
             tt_svq3_luma_dc_dequant_idct_c(h->mb + p * 256, h->mb_luma_dc[p], qscale);
#endif
    }
}

static ttv_always_inline void hl_decode_mb_idct_luma(H264Context *h, int mb_type,
...
//                     uint8_t *const ptr = dest_y + block_offset[i];
//                     tt_svq3_add_idct_c(ptr, h->mb + i * 16 + p * 256, linesize,
