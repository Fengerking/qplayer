...
TTInt CTTExtAudioFileReader::ReadSync(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
    OSStatus nErr = noErr;
    ...
    if (nErr == noErr)
    {
        GKASSERT(iCurBufferList.mNumberBuffers == 1);
        GKASSERT(iCurBufferList.mBuffers[0].mNumberChannels == iChannels);
...
TTInt64 CTTExtAudioFileReader::Size() const
{
	return 0;
}

TTInt CTTExtAudioFileReader::ReadWait(TTUint8* aReadBuffer, TTInt64 aReadPos, TTInt aReadSize)
{
    return 0;
}
...
TTInt CTTExtAudioFileReader::PrepareCache(TTInt64 aReadPos, TTInt aReadSize, TTInt aFlag)
{
