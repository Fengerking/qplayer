/**
* File : TTBaseAudioSink.cpp  
* Created on : 2014-9-1
* Author : yongping.lin
* Description : TTBaseAudioSink实现文件
*/
...
TTInt TTCBaseAudioSink::open(TTAudioInfo* pCurAudioInfo)
{
	...
	mMaxPCMCount = mAudioFormat.SampleRate*mAudioFormat.Channels*mAudioFormat.SampleBits/(8*2);
...
void TTCBaseAudioSink::setPlayStatus(GKPlayStatus aStatus)
{
...
GKPlayStatus TTCBaseAudioSink::getPlayStatus()
{
	GKCAutoLock Lock(&mCritStatus);
	GKPlayStatus status = mPlayStatus;
	return status;
}
